...
This commit was automatically generated by RSOLV to fix security vulnerabilities.`;
  ...
  /**
   * RFC-095 Compatibility: Generate solution with context gathering
   *
   * This is a compatibility method for the unified adapter interface.
   * The legacy adapter doesn't have SDK-based context gathering, so this
   * simply delegates to generateSolutionWithGit.
   */
  async generateSolutionWithContext(
    ...
    logger.info('[Legacy Adapter] generateSolutionWithContext delegating to generateSolutionWithGit');
    return this.generateSolutionWithGit(issueContext, analysis);
  }

  /**
   * RFC-095 Compatibility: Gather deep context from codebase
   *
   * This is a compatibility method for the unified adapter interface.
   * The legacy adapter doesn't have SDK-based context gathering capabilities,
   * so this returns an empty context object.
   */
  async gatherDeepContext(
    ...
    logger.info('[Legacy Adapter] gatherDeepContext not supported - returning empty context');
    return {
      files: [],
  ...
  /**
   * RFC-095 Compatibility: Generate solution (matches unified adapter interface)
   *
   * This method matches the ClaudeAgentSDKAdapter.generateSolution() signature
   * so that solution.ts can use the factory function and call either adapter
   * with the same interface.
   *
   * Delegates to generateSolutionWithGit which is the legacy implementation.
   */
  async generateSolution(
    ...
    logger.info('[Legacy Adapter] generateSolution delegating to generateSolutionWithGit');
    return this.generateSolutionWithGit(issueContext, analysis);
  }
}