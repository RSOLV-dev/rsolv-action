  ...
  try {
    ...
    while (iteration < maxIterations) {
      ...
      if ((config.testGeneration?.validateFixes || config.fixValidation?.enabled !== false) && 
        ...
        // CRITICAL FIX: Use the pre-generated executable testSuite from testResults,
        // NOT any test descriptions that might come from Claude's JSON response.
        // The testResults.generatedTests.testSuite contains actual executable test code
        // generated by TestGenerator, while Claude only provides test descriptions.
        const validation = await validator.validateFixWithTests(
          beforeFixCommit,
          solution.commitHash!,
          testResults.generatedTests.testSuite  // This is the executable test suite
        );
