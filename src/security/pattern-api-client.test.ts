...
describe('PatternAPIClient', () => {
  ...
  describe('Type mapping from API to VulnerabilityType', () => {
    ...
    test('should preserve CWE-94 for code_injection patterns', async () => {
      client = new PatternAPIClient({ apiKey: 'test-key' });
    ...
    test('should NOT map code_injection to COMMAND_INJECTION', async () => {
      client = new PatternAPIClient({ apiKey: 'test-key' });
      ...
      expect(result.patterns[0].type).toBe(VulnerabilityType.CODE_INJECTION);
      expect(result.patterns[0].type).not.toBe(VulnerabilityType.COMMAND_INJECTION);
    });

    test('should NOT map code_injection to IMPROPER_INPUT_VALIDATION (regression test)', async () => {
      client = new PatternAPIClient({ apiKey: 'test-key' });
      ...
      expect(result.patterns[0].type).not.toBe(VulnerabilityType.IMPROPER_INPUT_VALIDATION);
      expect(result.patterns[0].type).toBe(VulnerabilityType.CODE_INJECTION);
    });

    test('should preserve severity and recommendation through type mapping', async () => {
      client = new PatternAPIClient({ apiKey: 'test-key' });
    ...
    test('should compile regex patterns correctly for code_injection type', async () => {
      client = new PatternAPIClient({ apiKey: 'test-key' });
      ...
      expect(result.patterns[0].type).toBe(VulnerabilityType.CODE_INJECTION);
      expect(result.patterns[0].patterns.regex).toHaveLength(1);
      expect(result.patterns[0].patterns.regex[0]).toBeInstanceOf(RegExp);

      // Test that the regex actually works on the RailsGoat case
      const testCode = 'eval(request.responseText);';
      expect(result.patterns[0].patterns.regex[0].test(testCode)).toBe(true);
    });
  });
