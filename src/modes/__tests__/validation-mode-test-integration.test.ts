...
describe('ValidationMode - Test Integration Workflow', () => {
  ...
  describe('generateTestWithRetry() - RED test success on genuine failure', () => {
    it('should accept test on genuine assertion failure without regression check', async () => {
      // RFC-101 v3.8.70: Regression detection was disabled for RED test generation
      // because the entire test file is generated by us. The previous heuristic
      // (counting "failed" word occurrences) caused false positives in pytest/PHPUnit.
      // For RED tests, we only care if the test has a genuine assertion failure.
  ...
  it 'shows file' do
    ...
    expect(response).to be_successful
      ...
      // Mock test execution to show failure (which is good for RED tests!)
      let testRunCount = 0;
      (execSync as any).mockImplementation((cmd: string) => {
        if (cmd.includes('ruby -c')) return '';
        if (cmd.includes('bundle exec rspec')) {
          testRunCount++;
          // Test fails with genuine assertion error - should succeed on first attempt
...
1 test failed, 0 passed`;
      ...
      expect(result).toBeDefined();
      expect(result).not.toBeNull();
      // Should succeed on first attempt (no regression retry needed)
      expect(testRunCount).toBe(1);
    });
  });
