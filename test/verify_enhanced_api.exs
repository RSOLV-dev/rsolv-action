  ...
  defp check_patterns_for_ast_data(patterns) do
    ...
    Enum.each(patterns, fn pattern ->
      if pattern["id"] in known_enhanced do
        ...
        has_context_rules = Map.has_key?(pattern, "context_rules") && pattern["context_rules"] != nil
        has_confidence_rules = Map.has_key?(pattern, "confidence_rules") && pattern["confidence_rules"] != nil
        has_min_confidence = Map.has_key?(pattern, "min_confidence") && pattern["min_confidence"] != nil
        ...
        if has_ast_rules || has_context_rules || has_confidence_rules do
          ...
          if has_ast_rules, do: IO.puts("     - ast_rules: #{inspect(Map.keys(pattern["ast_rules"]))}")
          if has_context_rules, do: IO.puts("     - context_rules: #{inspect(Map.keys(pattern["context_rules"]))}")
          if has_confidence_rules, do: IO.puts("     - confidence_rules: present")
          if has_min_confidence, do: IO.puts("     - min_confidence: #{pattern["min_confidence"]}")
        else
