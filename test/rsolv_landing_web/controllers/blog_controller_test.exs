defmodule RsolvLandingWeb.BlogControllerTest do
  ...
  describe "when blog feature flag is enabled" do
    ...
    test "GET /blog/:slug shows blog post if it exists", %{conn: conn} do
      ...
      posts = RsolvLandingWeb.Services.BlogService.list_posts()
  ...
  describe "blog post metadata extraction" do
    test "extracts frontmatter from markdown files" do
      ...
      if length(posts) > 0 do
        post = hd(posts)
        assert {:ok, fetched_post} = RsolvLandingWeb.Services.BlogService.get_post(post.slug)
    ...
    test "handles missing blog posts" do
      assert {:error, :not_found} = RsolvLandingWeb.Services.BlogService.get_post("nonexistent")
    end
  ...
  describe "draft/published post visibility" do
    ...
    test "RSS feed only includes published posts" do
      # Generate RSS feed
      rss = RsolvLandingWeb.Services.BlogService.generate_rss()
      ...
      all_posts = RsolvLandingWeb.Services.BlogService.list_all_posts_unfiltered()
  ...
  describe "MDEx markdown processing" do
    test "renders markdown with syntax highlighting", %{conn: conn} do
      ...
      case RsolvLandingWeb.Services.BlogService.get_post("syntax-highlighting-showcase") do
        {:ok, post} ->
    ...
    test "removes duplicate H1 titles from markdown", %{conn: conn} do
      ...
      case RsolvLandingWeb.Services.BlogService.get_post("from-zero-to-23-vulnerabilities") do
        {:ok, post} ->
    ...
    test "MDEx extensions are enabled", %{conn: conn} do
      ...
      case RsolvLandingWeb.Services.BlogService.get_post("ai-security-in-practice") do
        {:ok, post} ->
