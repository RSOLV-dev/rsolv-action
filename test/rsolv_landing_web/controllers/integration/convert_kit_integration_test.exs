defmodule RsolvLandingWeb.ConvertKitIntegrationTest do
  use RsolvWeb.ConnCase
  
  # Import Mox for setting up mock expectations
  import Mox
  alias RsolvLanding.HTTPClientMock
  ...
  describe "early access form integration" do
    ...
    test "validates email before sending to ConvertKit", %{conn: conn} do
      ...
      signup = RsolvLanding.EarlyAccess.get_signup_by_email("invalid-email")
      assert signup == nil
    end
    ...
    test "handles different email param formats", %{conn: conn, fixtures: fixtures} do
      ...
      assert RsolvLanding.EarlyAccess.email_exists?("liveview@example.com")
      assert RsolvLanding.EarlyAccess.email_exists?("oldform@example.com")
    end
  end
end