defmodule RsolvLandingWeb.Plugs.DashboardAuthEnhancedTest do
  use RsolvWeb.ConnCase
  alias RsolvLandingWeb.Plugs.DashboardAuthEnhanced
  
  setup do
    # Store original config
    original_password = Application.get_env(:rsolv_landing, :admin_password)
    original_emails = Application.get_env(:rsolv_landing, :admin_emails)
    
    # Set test config
    Application.put_env(:rsolv_landing, :admin_password, "test123")
    Application.put_env(:rsolv_landing, :admin_emails, ["admin@test.com"])
    
    on_exit(fn ->
      # Restore original config
      Application.put_env(:rsolv_landing, :admin_password, original_password)
      Application.put_env(:rsolv_landing, :admin_emails, original_emails)
    end)
