defmodule Rsolv.ValidationCacheHelpers do
  ...
  def build_validation_data(opts) do
    ...
    file_hashes = Keyword.get_lazy(opts, :file_hashes, fn ->
      locations
      |> Enum.map(& &1.file_path)
      |> Enum.uniq()
      |> Map.new(fn path -> {path, "sha256:#{:crypto.hash(:sha256, path) |> Base.encode16()}"} end)
    end)
  ...
  def calculate_cache_stats(operations) do
    stats = Enum.reduce(operations, %{hits: 0, misses: 0, expired: 0, invalidated: 0}, fn
