defmodule RsolvWeb.FeedbackDashLiveTest do
  ...
  describe "FeedbackDashLive connection tests" do
    test "establishes WebSocket connection successfully", %{conn: conn} do
      result =
        test_websocket_lifecycle(conn, "/feedback-dashboard", fn view ->
  ...
  describe "FeedbackDashLive performance tests" do
    test "measures WebSocket performance", %{conn: conn} do
      {:ok, metrics} =
        measure_websocket_performance(conn, "/feedback-dashboard", 3, fn view ->
      ...
      # Should connect in less than 1 second
      assert metrics.connect_time < 1000
      # Average event should process in less than 500ms
      assert metrics.avg_event_time < 500
      # Should disconnect in less than 1 second
      assert metrics.disconnect_time < 1000
