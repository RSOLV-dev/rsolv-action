defmodule RsolvWeb.RoiCalculatorLiveTest do
  ...
  describe "ROI Calculator LiveComponent" do
    test "renders on homepage with default values", %{conn: conn} do
      ...
      # Updated to match new defaults: 5.5 dev + 1.5 coordination
      assert html =~ "6 hours"
      # Updated to match new defaults: $130K salary + overhead
      assert html =~ "$85/hour"
      assert html =~ "80% deployed"

      # Verify default calculations
      # fixes deployed (50 * 0.8)
      assert html =~ "40/month"
      # time saved (40 * 6)
      assert html =~ "240 hours"
      # pay as you go cost
      assert html =~ "$600"
      # teams plan cost
      assert html =~ "$499"
