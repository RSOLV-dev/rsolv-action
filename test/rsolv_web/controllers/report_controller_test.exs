defmodule RsolvWeb.ReportControllerTest do
  use RsolvWeb.ConnCase, async: false  # Don't run async due to FunWithFlags database issues
  ...
  setup do
    ...
    Process.sleep(10) # Small delay to ensure database consistency
  ...
  describe "download/2" do
    test "generates CSV report with default parameters", %{conn: conn} do
      ...
      log = capture_log(fn ->
        ...
        assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-conversions-30d.csv"]
    ...
    test "generates JSON report when format is json", %{conn: conn} do
      log = capture_log(fn ->
        ...
        assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-conversions-30d.json"]
        assert response(conn, 200)
      end)
    ...
    test "handles different report types", %{conn: conn} do
      report_types = ["conversions", "page_views", "traffic", "form_events", "engagement", "signup"]
      ...
      for report_type <- report_types do
        log = capture_log(fn ->
          ...
          assert response(conn, 200)
          assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-#{report_type}-30d.csv"]
        end)
    ...
    test "handles different time periods", %{conn: conn} do
      ...
      for period <- periods do
        log = capture_log(fn ->
          ...
          assert response(conn, 200)
          assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-conversions-#{period}.csv"]
        end)
    ...
    test "combines parameters correctly", %{conn: conn} do
      log = capture_log(fn ->
        ...
        assert response_content_type(conn, :json)
        assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-page_views-7d.json"]
        assert response(conn, 200)
      end)
    ...
    test "handles unknown report type", %{conn: conn} do
      ...
      assert response(conn, 200)
      assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-unknown-30d.csv"]
    end
    ...
    test "handles invalid period", %{conn: conn} do
      ...
      assert response(conn, 200)
      assert get_resp_header(conn, "content-disposition") == ["attachment; filename=rsolv-analytics-conversions-invalid.csv"]
    end
  end
end