defmodule RsolvWeb.CredentialControllerTest do
  ...
  describe "POST /api/v1/credentials/exchange" do
    ...
    test "rate limiter is configured and working", %{
      ...
      assert {:ok, _metadata} = result = Rsolv.RateLimiter.check_rate_limit(customer.id, :credential_exchange)
      assert result == :ok
    end
    ...
    setup %{customer: customer} do
      ...
      {:ok, credential} =
        Credentials.create_temporary_credential(%{
          ...
          provider: "anthropic",
          # 4 minutes 10 seconds - eligible for refresh
          expires_at: DateTime.add(DateTime.utc_now(), 250),
          usage_limit: 100
        })
    ...
    test "returns 403 for credential owned by another customer", %{
      ...
      {:ok, credential} =
        Credentials.create_temporary_credential(%{
          ...
          provider: "anthropic",
          # Eligible for refresh like main test
          expires_at: DateTime.add(DateTime.utc_now(), 250)
        })
