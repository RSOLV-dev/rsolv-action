defmodule RsolvWeb.Api.V1.TestIntegrationControllerTest do
  ...
  describe "POST /api/v1/test-integration/analyze" do
    test "requires authentication", %{conn: conn} do
      conn = post(conn, "/api/v1/test-integration/analyze", %{
        vulnerableFile: "app/model.rb",
    ...
    test "recommends best test file for integration based on semantic similarity", %{conn: conn, api_key: api_key} do
      ...
      conn = conn
        ...
        candidateTestFiles: [
          "spec/controllers/users_controller_spec.rb",  # Direct unit test - should score HIGHEST
          "spec/requests/users_spec.rb",                 # Integration test - should score MEDIUM
          "spec/models/user_spec.rb"                     # Different component - should score LOWEST
        ],
        framework: "rspec"
      })
      ...
      assert best["reason"] =~ ~r/(direct|unit|controller)/i, "Reason should explain why this is best choice"

      # Desired behavior: Scores are in descending order (best first)
      scores = Enum.map(recommendations, & &1["score"])
      assert scores == Enum.sort(scores, :desc), "Recommendations must be sorted by score descending"
      ...
      assert score_gap >= 0.3, "Direct unit test should score at least +0.3 higher than unrelated test (module bonus)"

      # Desired behavior: Fallback provides new file path when no good match exists
      assert fallback["path"] =~ ~r/spec\/security/, "Fallback should suggest security-specific test file"
      assert fallback["reason"], "Fallback must explain why it's needed"
    end

    test "prioritizes unit tests over integration tests for JavaScript/TypeScript", %{conn: conn, api_key: api_key} do
      ...
      conn = conn
        ...
        candidateTestFiles: [
          "test/controllers/AuthController.test.ts",  # Direct unit test - closer path match
          "test/integration/auth.test.ts"              # Integration test - different directory
        ],
        framework: "jest"
      })
      ...
      assert best["score"] > second["score"], "Unit test should score higher than integration test"
  ...
  describe "POST /api/v1/test-integration/generate" do
    test "requires authentication", %{conn: conn} do
      conn = post(conn, "/api/v1/test-integration/generate", %{
    ...
    test "integrates security test into existing test file while preserving structure", %{conn: conn, api_key: api_key} do
