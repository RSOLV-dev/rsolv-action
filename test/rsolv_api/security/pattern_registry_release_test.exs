defmodule RsolvApi.Security.PatternRegistryReleaseTest do
  use ExUnit.Case
  ...
  describe "pattern loading in release environment" do
    test "loads Python patterns without filesystem access" do
      ...
      assert "python-command-injection-os-system" in pattern_ids, "Should include command injection pattern"
    end
    ...
    test "loads common patterns that apply to all languages" do
      ...
      common_pattern_ids = all_patterns 
        |> Enum.filter(fn p -> 
      ...
      assert length(common_pattern_ids) > 0, "Should include common patterns like JWT, weak crypto, or hardcoded secrets"
    end
  ...
  describe "get_all_patterns/0" do
    ...
    test "loads PHP patterns specifically" do
      ...
      assert Enum.any?(pattern_ids, &String.starts_with?(&1, "php-")), "PHP patterns should have php- prefix"
    end
  end
end