defmodule RsolvApi.Security.ASTPatternTest do
  use ExUnit.Case, async: true
  
  alias RsolvApi.Security.{Pattern, ASTPattern}
  ...
  describe "enhance/1" do
    ...
    test "nosql injection pattern checks for dangerous operators" do
      ...
      assert enhanced.ast_rules.argument_contains.dangerous_keys == ["$where", "$expr", "$function"]
  ...
  describe "false positive reduction" do
    test "sql injection pattern excludes test files" do
      ...
      assert enhanced.context_rules.exclude_paths |> Enum.any?(&Regex.match?(&1, "test/models/user_test.js"))
    end
