defmodule RsolvApi.Security.PatternsTest do
  use ExUnit.Case, async: true
  
  alias RsolvApi.Security.Pattern
  alias RsolvApi.Security.Patterns
  ...
  describe "Pattern modules exist" do
    ...
    for language <- @languages do
      test "#{language} pattern module exists" do
        module = Module.concat(RsolvApi.Security.Patterns, unquote(language |> to_string() |> Macro.camelize()))
  ...
  describe "JavaScript patterns" do
    test "module exports sql_injection_concat/0" do
      assert function_exported?(RsolvApi.Security.Patterns.Javascript, :sql_injection_concat, 0)
    end
    ...
    test "sql_injection_concat pattern structure is valid" do
      pattern = RsolvApi.Security.Patterns.Javascript.sql_injection_concat()
