defmodule RsolvApi.Security.Patterns.Rails.CallbackSecurityBypassTest do
  ...
  describe "pattern/0" do
    test "returns valid pattern structure" do
      ...
      assert pattern.description == "Bypass of security constraints through skip callback conditions"
  ...
  describe "applies_to_file?/2" do
    test "applies to Rails controller files" do
      assert CallbackSecurityBypass.applies_to_file?("app/controllers/users_controller.rb", ["rails"])
      assert CallbackSecurityBypass.applies_to_file?("app/controllers/admin/settings_controller.rb", ["rails"])
      assert CallbackSecurityBypass.applies_to_file?("app/controllers/application_controller.rb", ["rails"])
      assert CallbackSecurityBypass.applies_to_file?("app/controllers/api/v1/base_controller.rb", ["rails"])
    end

    test "applies to concerns" do
      assert CallbackSecurityBypass.applies_to_file?("app/controllers/concerns/authenticatable.rb", ["rails"])
      assert CallbackSecurityBypass.applies_to_file?("app/controllers/concerns/authorization.rb", ["rails"])
    end
    ...
    test "does not apply to non-controller files" do
      ...
      refute CallbackSecurityBypass.applies_to_file?("test/controllers/users_controller_test.rb", ["rails"])
    end
