defmodule RsolvApi.Security.Patterns.Elixir.EtsPublicTableTest do
  ...
  describe "ets_public_table pattern" do
    ...
    test "does not detect protected or private tables" do
      ...
      safe_code = [
        ...
        ":ets.new(:config, [:protected])",  # Default is protected
        ":ets.new(:stats, [])"  # No explicit access level (defaults to protected)
      ]
    ...
    test "does not detect safe public table usage" do
      ...
      safe_code = [
        "# ETS table with :public access",  # Comments
        ":ets.lookup(public_table, key)",  # Table access, not creation
        ":ets.insert(existing_table, data)"  # Operations on existing tables
      ]
