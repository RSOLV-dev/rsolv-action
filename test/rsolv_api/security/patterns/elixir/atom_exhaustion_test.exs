defmodule RsolvApi.Security.Patterns.Elixir.AtomExhaustionTest do
  ...
  describe "atom_exhaustion pattern" do
    ...
    test "does not detect safe atom usage" do
      ...
      safe_code = [
        "Jason.decode!(user_input)",  # No keys: :atoms
        "Jason.decode(data, keys: :atoms!)",  # Safe :atoms! option
        "String.to_existing_atom(known_value)",  # Uses existing atoms only
        "binary_to_existing_atom(validated_input)",  # Safe alternative
        ":hardcoded_atom",  # Hardcoded atoms are safe
        "Enum.map(~w[one two three]a, &process/1)"  # Compile-time atoms
      ]
    ...
    test "does not detect safe JSON decoding patterns" do
      ...
      safe_code = [
        "Jason.decode!(json_string)",  # String keys (default)
        "Jason.decode(data, keys: :strings)",  # Explicit string keys
        "Poison.decode!(json, as: %User{})",  # Struct decoding
        "# This comment mentions Jason.decode with keys: :atoms"  # Comments
      ]
