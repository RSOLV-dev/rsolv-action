defmodule RsolvApi.Security.Patterns.Ruby.EvalUsageTest do
  ...
  describe "regex matching" do
    ...
    test "does not match safe eval patterns", %{pattern: pattern} do
      safe_code = [
        ~S|eval("puts 'hello world'")|,  # Static string
        ~S|instance_eval { @name = 'safe' }|,  # Block syntax
        ~S|class_eval do; attr_reader :id; end|,  # Block with do..end
        ~S|send(:valid_method)|,  # Symbol instead of dynamic input
        ~S|const_get("SAFE_CONSTANT")|,  # Static string
        ~S|evaluate_something(params[:data])|,  # Different method
        ~S|evaluation = params[:score]|  # Variable assignment
      ]
