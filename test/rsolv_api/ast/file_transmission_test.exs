defmodule RsolvApi.AST.FileTransmissionTest do
  ...
  describe "prepare_for_transmission/2" do
    test "prepares small file for transmission", %{session: session} do
      ...
      assert String.length(result.metadata.content_hash) == 64  # SHA256 hex
    end
  ...
  describe "receive_transmission/3" do
    test "reassembles chunked file correctly", %{session: session} do
      ...
      {:ok, received} = FileTransmission.receive_transmission(
        prepared.chunks,
    ...
    test "detects tampered chunks", %{session: session} do
      ...
      assert {:error, :decryption_failed} = FileTransmission.receive_transmission(
        tampered_chunks,
        prepared.metadata.content_hash,
        session
      )
    ...
    test "detects wrong hash", %{session: session} do
      ...
      assert {:error, :integrity_check_failed} = FileTransmission.receive_transmission(
        prepared.chunks,
        wrong_hash,
        session
      )
  ...
  describe "decrypt_file/2" do
    ...
    test "fails with wrong session key", %{session: session} do
      ...
      assert {:error, :decryption_failed} = FileTransmission.decrypt_file(encrypted_file, wrong_session)
    end
    ...
    test "detects hash mismatch", %{session: session} do
      ...
      assert {:error, :integrity_check_failed} = FileTransmission.decrypt_file(encrypted_file, session)
    end
  end
  ...
  describe "stream_file/2" do
    test "streams file in chunks", %{session: session} do
      ...
      try do
        ...
        # Stream the file
        chunks = FileTransmission.stream_file(temp_path, session)
                |> Enum.to_list()
