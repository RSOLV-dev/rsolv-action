defmodule RsolvApi.AST.ASTCacheTest do
  use ExUnit.Case, async: true
  
  alias RsolvApi.AST.ASTCache
  ...
  describe "memory management" do
    ...
    test "evicts entries when memory limit exceeded" do
      ...
      large_data = String.duplicate("x", 100_000)  # ~100KB
      
      for i <- 1..15 do  # Should exceed 1MB
  ...
  describe "cache warming" do
    ...
    test "pre-warms frequently accessed files", %{cache: cache} do
      ...
      files_for_warming = Enum.map(files_with_hashes, fn {hash, lang, file} ->
        {hash, lang}
      end)
      
      warmed_count = ASTCache.warm_cache(cache, files_for_warming, fn {file_hash, language} ->
        # Find original file name for this hash
        {_, _, original_file} = Enum.find(files_with_hashes, fn {hash, lang, _} ->
    ...
    test "skips warming for already cached files", %{cache: cache} do
      ...
      warmed_count = ASTCache.warm_cache(cache, [{file_hash, "javascript"}], fn _ ->
        %{"type" => "ShouldNotSee"}
      end)
  ...
  describe "concurrent access" do
    ...
    test "handles concurrent reads and writes", %{cache: cache} do
      ...
      tasks = for i <- 1..20 do
        Task.async(fn ->
          file_hash = "concurrent_#{rem(i, 5)}"  # 5 unique keys
