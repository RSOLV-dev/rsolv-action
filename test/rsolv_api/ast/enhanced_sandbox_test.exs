defmodule RsolvApi.AST.EnhancedSandboxTest do
  use ExUnit.Case, async: true
  
  alias RsolvApi.AST.EnhancedSandbox
  ...
  describe "input validation" do
    ...
    test "detects shell injection attempts" do
      ...
      assert {:error, {:suspicious_pattern, _}} = EnhancedSandbox.validate_input(malicious_code, "javascript")
    end
    ...
    test "detects eval usage" do
      ...
      assert {:error, {:suspicious_pattern, _}} = EnhancedSandbox.validate_input(code_with_eval, "javascript")
    end
    ...
    test "detects excessive nesting" do
      ...
      assert {:error, {:suspicious_pattern, _}} = EnhancedSandbox.validate_input(nested, "javascript")
    end
    ...
    test "detects Python-specific dangerous patterns" do
      ...
      assert {:error, {:suspicious_pattern, _}} = EnhancedSandbox.validate_input(python_code, "python")
    end
    ...
    test "detects Ruby-specific dangerous patterns" do
      ...
      assert {:error, {:suspicious_pattern, _}} = EnhancedSandbox.validate_input(ruby_code, "ruby")
    end
    ...
    test "detects high complexity code" do
      ...
      complex_input = Enum.reduce(1..50, "", fn i, acc ->
        acc <> "function f#{i}() { " <> String.duplicate("x", 1000) <> " }\n"
      end) <> String.duplicate("var y = ", 100)
      
      assert {:error, {:complexity_too_high, _}} = EnhancedSandbox.validate_input(complex_input, "javascript")
    end
  end
  ...
  describe "enhanced configuration" do
    test "creates stricter configuration than base" do
      ...
      assert config.limits.max_heap_size == 32_000_000  # 128MB
