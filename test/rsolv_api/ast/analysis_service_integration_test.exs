defmodule RsolvApi.AST.AnalysisServiceIntegrationTest do
  use ExUnit.Case, async: false
  
  alias RsolvApi.AST.{AnalysisService, SessionManager}
  ...
  describe "performance and error handling" do
    ...
    test "provides performance metrics", %{session: _session} do
      ...
      {:ok, _findings, metrics} = AnalysisService.analyze_file_with_metrics(
        file, 
  ...
  describe "batch analysis" do
    test "analyzes multiple files in parallel", %{session: session} do
      ...
      {:ok, results} = AnalysisService.analyze_batch(
        files,
        %{"includeSecurityPatterns" => true},
        session
      )
