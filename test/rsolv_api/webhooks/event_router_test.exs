defmodule RsolvApi.Webhooks.EventRouterTest do
  ...
  describe "route_event/3" do
    ...
    test "returns error for unsupported platform" do
      ...
      assert {:error, :unsupported_platform} = EventRouter.route_event("bitbucket", headers, payload)
    end
    ...
    test "handles GitLab events (future implementation)" do
      ...
      assert {:error, :platform_not_implemented} = EventRouter.route_event("gitlab", headers, payload)
    end
  end
  ...
  describe "verify_signature/3" do
    test "verifies GitHub webhook signature" do
      secret = "test_secret"
      payload = ~s({"test": "data"})
      signature = "sha256=" <> Base.encode16(:crypto.mac(:hmac, :sha256, secret, payload), case: :lower)
    ...
    test "rejects invalid GitHub signature" do
      ...
      assert {:error, :invalid_signature} = EventRouter.verify_signature("github", invalid_signature, payload, secret)
    end

    test "handles missing signature" do
      assert {:error, :missing_signature} = EventRouter.verify_signature("github", nil, "{}", "secret")
    end
  end
