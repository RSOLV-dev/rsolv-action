defmodule Rsolv.Security.Patterns.Rails.UnsafeRouteConstraintsTest do
  ...
  describe "unsafe_route_constraints pattern" do
    ...
    test "does not detect safe constraint usage" do
      ...
      safe_code = [
        ...
        # Environment check
        "constraints: -> { Rails.env.production? }",
        # Anchored numeric pattern
        "constraints: { id: /\\A\\d+\\z/ }",
        # Commented code
        "# constraints: { id: /.*/ } - commented out",
        # Static host
        "constraints: { host: 'example.com' }",
        # Whitelist check
        "constraints proc { |req| ALLOWED_HOSTS.include?(req.host) }"
      ]
    ...
    test "applies to Ruby files" do
      ...
      assert UnsafeRouteConstraints.applies_to_file?(
