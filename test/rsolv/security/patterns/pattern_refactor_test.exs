defmodule Rsolv.Security.Patterns.PatternRefactorTest do
  ...
  describe "refactored pattern structure" do
    ...
    test "pattern includes structured vulnerability metadata" do
      ...
      # :cwe, :owasp, :nist, :research, :blog, etc.
      assert Map.has_key?(first_ref, :type)
      # CWE-89, A03:2021, etc.
      assert Map.has_key?(first_ref, :id)
      # Direct link
      assert Map.has_key?(first_ref, :url)
      # Human-readable title
      assert Map.has_key?(first_ref, :title)
