defmodule Rsolv.Security.Patterns.Javascript.UnsafeRegexTest do
  use ExUnit.Case, async: true

  alias Rsolv.Security.Patterns.Javascript.UnsafeRegex
  ...
  defp simple_quantifiers do
    [
      ...
      "/x+y/.test(input)",
      "const pattern = /a*b/",
      # Possessive quantifier (conceptual)
      "/a++b/.test(input)"
    ]
  end
  ...
  describe "UnsafeRegex pattern" do
    test "returns correct pattern structure with all required fields" do
      ...
      assert pattern.description ==
               "Regex with nested quantifiers can cause exponential backtracking"
