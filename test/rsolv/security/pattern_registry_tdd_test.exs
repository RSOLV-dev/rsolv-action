defmodule Rsolv.Security.PatternRegistryTDDTest do
  ...
  describe "TDD: Fix pattern loading issues" do
    ...
    test "JavaScript patterns should use 'js-' prefix, not 'javascript-'" do
      ...
      sql_pattern =
        Enum.find(patterns, fn p -> String.contains?(p.id, "sql-injection-concat") end)
    ...
    test "PHP patterns should be loaded successfully" do
      ...
      assert length(patterns) > 0,
             "Should load PHP patterns through registry, got #{length(patterns)}"

      # At minimum, we should have the XSS echo pattern
      xss_pattern =
        Enum.find(patterns, fn p ->
          String.contains?(p.id, "xss") && String.contains?(p.id, "echo")
        end)
    ...
    test "Common patterns should be loaded and accessible" do
      ...
      hardcoded_patterns =
        all_patterns |> Enum.filter(fn p -> String.contains?(p.id, "hardcoded") end)
    ...
    test "All major languages should have patterns loaded" do
      ...
      Enum.each(languages, fn lang ->
        ...
        if lang in ["php", "ruby", "java", "elixir"] do
          ...
          assert length(patterns) >= 0,
                 "Should have patterns for #{lang}, but got #{length(patterns)}"
        else
        ...
        if length(patterns) > 0 do
          ...
          has_language_specific =
            case lang do
              "javascript" ->
                Enum.any?(pattern_ids, &String.starts_with?(&1, "js-"))

              "python" ->
                Enum.any?(pattern_ids, &String.starts_with?(&1, "python-")) ||
                  Enum.any?(pattern_ids, &String.starts_with?(&1, "py-"))

              "ruby" ->
                Enum.any?(pattern_ids, &String.starts_with?(&1, "ruby-")) ||
                  Enum.any?(pattern_ids, &String.starts_with?(&1, "rb-"))

              "php" ->
                Enum.any?(pattern_ids, &String.starts_with?(&1, "php-"))

              "java" ->
                Enum.any?(pattern_ids, &String.starts_with?(&1, "java-"))

              "elixir" ->
                Enum.any?(pattern_ids, &String.starts_with?(&1, "elixir-")) ||
