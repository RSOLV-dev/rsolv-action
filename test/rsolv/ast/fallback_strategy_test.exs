defmodule Rsolv.AST.FallbackStrategyTest do
  use ExUnit.Case, async: false

  alias Rsolv.AST.{FallbackStrategy, ParserRegistry, SessionManager}
  ...
  describe "fallback strategy for AST parsing failures" do
    test "returns basic structure analysis when AST parsing fails", %{
      ...
      {:ok, result} =
        FallbackStrategy.analyze_with_fallback(
          session_id,
          customer_id,
          "javascript",
          invalid_code
    ...
    test "fallback detects common security patterns without AST", %{
      session_id: session_id,
      customer_id: customer_id
    } do
      vulnerable_code = """
      ...
      {:ok, result} =
        FallbackStrategy.analyze_with_fallback(
          ...
          customer_id,
          "javascript",
          # Add syntax error to force fallback
          vulnerable_code <> "const x = ;"
        )
      ...
      sql_injection =
        Enum.find(result.fallback_analysis.patterns_detected, fn p ->
          p.type == "sql_injection"
        end)

      assert sql_injection != nil
      # Lower confidence without AST
      assert sql_injection.confidence < 1.0
      # But still reasonable
      assert sql_injection.confidence >= 0.6
    end

    test "fallback handles parser timeout gracefully", %{
      session_id: session_id,
      customer_id: customer_id
    } do
      ...
      assert result.fallback_analysis != nil
      # Low confidence for timeout
      assert result.fallback_analysis.confidence < 0.5
    end

    test "fallback handles parser crash gracefully", %{
      session_id: session_id,
      customer_id: customer_id
    } do
    ...
    test "fallback calculates reasonable confidence scores" do
      ...
      {:ok, complex_result} =
        FallbackStrategy.analyze_with_fallback(
          ...
          "javascript",
          # syntax error
          complex_code <> " const err = ;"
        )

      # More complex code should have lower confidence in fallback
      assert simple_result.fallback_analysis.confidence >
               complex_result.fallback_analysis.confidence
    end
  ...
  describe "fallback strategy error handling" do
    ...
    test "handles analysis errors gracefully" do
      ...
      # Deeply nested
      problematic_code = String.duplicate("(", 10000)
