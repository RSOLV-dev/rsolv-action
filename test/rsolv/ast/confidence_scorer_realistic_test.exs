  ...
  describe "critical vulnerabilities without detected user input" do
    ...
    test "eval() should be high confidence even without detected user input" do
      ...
      context = %{
        ...
        function_name: "eval",
        has_user_input: false,  # Detection failed but it's still eval!
        ast_match: :exact
      }
    ...
    test "exec() command injection should be high confidence" do
      ...
      context = %{
        ...
        has_user_input: false,  # Detection might fail
        ast_match: :exact
      }
      
      confidence = ConfidenceScorer.calculate_confidence(context, "javascript", %{})
      
      assert confidence >= 0.70, "Command injection should have confidence >= 0.70, got #{confidence}"
    end
  end
  ...
  describe "patterns that don't require user input" do
    ...
    test "hardcoded secrets should have high confidence without user input" do
      ...
      context = %{
        ...
        has_user_input: false,  # Correct! Secrets don't have user input
        ast_match: :exact
      }
      ...
      assert confidence >= 0.80, "Hardcoded secrets should have confidence >= 0.80, got #{confidence}"
    end
    ...
    test "weak crypto should have reasonable confidence without user input" do
      ...
      context = %{
        ...
        has_user_input: false,  # Crypto doesn't need user input
        ast_match: :exact
      }
    ...
    test "insecure random should have reasonable confidence" do
      context = %{
        ...
        has_user_input: false,  # Math.random() doesn't need user input
        ast_match: :exact
      }
      
      confidence = ConfidenceScorer.calculate_confidence(context, "javascript", %{})
      
      assert confidence >= 0.60, "Insecure random should have confidence >= 0.60, got #{confidence}"
    end
  end
  ...
  describe "user input detection uncertainty" do
    ...
    test "SQL injection with uncertain user input should still be reportable" do
      ...
      context = %{
        ...
        has_user_input: false,  # Uncertain detection
  ...
  describe "false positive reduction for safe patterns" do
    test "framework protection should significantly reduce confidence" do
      ...
      context = %{
        pattern_type: :sql_injection,
        has_user_input: true,
        framework_protection: true,  # Using ORM
        ast_match: :exact
      }
