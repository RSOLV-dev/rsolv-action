  ...
  describe "end-to-end AST pattern matching" do
    test "detects SQL injection with high confidence and proper context" do
      ...
      pattern =
        Enum.find(
          patterns,
          &(String.contains?(&1.id, "sql") && String.contains?(&1.id, "fstring"))
        )
      ...
      assert match.confidence >= Map.get(pattern, :min_confidence, 0.7) * 0.7
      # ASTPatternMatcher determines severity
    ...
    test "reduces confidence for test files" do
      ...
      # Very low confidence in tests
      assert test_confidence < 0.5
    end

    test "enhances traditional patterns with AST rules" do
      # Start with a basic pattern
      basic_pattern = %Rsolv.Security.Pattern{
        id: "js-sql-injection",
    ...
    test "integrates with safe context detection" do
      ...
      context =
        Rsolv.AST.ContextAnalyzer.analyze_code(
      ...
      param_context =
        Rsolv.AST.ContextAnalyzer.evaluate_security_context(
