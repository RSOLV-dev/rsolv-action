defmodule Rsolv.AST.PatternIntegrationTest do
  ...
  describe "end-to-end AST pattern matching" do
    ...
    test "integrates with safe context detection" do
      ...
      context = Rsolv.AST.ContextAnalyzer.analyze_code(
      ...
      param_context = Rsolv.AST.ContextAnalyzer.evaluate_security_context(
      ...
      unsafe_context = Rsolv.AST.ContextAnalyzer.evaluate_security_context(
        "db.query('SELECT * FROM users WHERE id = ' + id)", 
        "javascript",
        %{path: "app/controllers/users.js", pattern_type: :sql_injection}
      )
