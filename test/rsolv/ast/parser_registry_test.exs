defmodule Rsolv.AST.ParserRegistryTest do
  ...
  setup do
    ...
    case GenServer.whereis(ParserRegistry) do
      # ParserRegistry is started by Application
      nil -> :ok
      _ -> :ok
    end
  ...
  describe "parser routing" do
    ...
    test "handles parsing errors gracefully" do
      ...
      {:ok, result} =
        ParserRegistry.parse_code(session.id, customer_id, "javascript", invalid_code)
  ...
  describe "error handling" do
    test "handles parser crash gracefully" do
      ...
      {:ok, result2} =
        ParserRegistry.parse_code(session.id, customer_id, "javascript", normal_code)
    ...
    test "enforces parser timeout" do
      ...
      {:ok, result} =
        ParserRegistry.parse_code(session.id, customer_id, "javascript", timeout_code)
