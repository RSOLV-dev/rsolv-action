defmodule Rsolv.AST.EncryptionBenchmarkTest do
  ...
  test "encryption performance benchmark" do
    ...
    for {size, label} <- sizes do
      ...
      {encrypt_time, {ciphertext, iv, auth_tag}} =
        :timer.tc(fn ->
          Encryption.encrypt(data, key)
        end)

      # Benchmark decryption
      {decrypt_time, {:ok, _plaintext}} =
        :timer.tc(fn ->
    ...
    # Just first 3 sizes
    for {size, label} <- Enum.take(sizes, 3) do
      data = :crypto.strong_rand_bytes(size)

      {prepare_time, {:ok, prepared}} =
        :timer.tc(fn ->
          FileTransmission.prepare_for_transmission(data, session)
        end)

      {receive_time, {:ok, _received}} =
        :timer.tc(fn ->
