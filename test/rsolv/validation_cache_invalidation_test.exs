defmodule Rsolv.ValidationCacheInvalidationTest do
  ...
  describe "invalidate/2" do
    ...
    test "accepts different invalidation reasons" do
      ...
      {:ok, cache1} = ValidationCache.store(build_validation_data(forge_account, %{
      ...
      {:ok, cache2} = ValidationCache.store(build_validation_data(forge_account, %{
      ...
      # Test manual reason
      {:ok, cache3} = ValidationCache.store(build_validation_data(forge_account, %{
    ...
    test "returns error for non-existent cache entry" do
      result = ValidationCache.invalidate(999999, "manual")
  ...
  describe "invalidate_by_file/3" do
    test "invalidates all entries containing a specific file" do
      ...
      {:ok, _cache1} = ValidationCache.store(build_validation_data(forge_account, %{
        vulnerability_type: "sql-injection",
        locations: [%{file_path: "app.js", line: 42}]
      }))
      
      {:ok, _cache2} = ValidationCache.store(build_validation_data(forge_account, %{
        vulnerability_type: "xss",
    ...
    test "handles multi-file vulnerabilities correctly" do
      ...
      {:ok, _multi} = ValidationCache.store(build_validation_data(forge_account, %{
        locations: [
          %{file_path: "app.js", line: 42},
  ...
  describe "invalidate_by_repository/2" do
    test "invalidates all entries for a repository" do
      ...
      {:ok, count} = ValidationCache.invalidate_by_repository(
        forge_account.id,
        "RSOLV-dev/nodegoat"
      )
