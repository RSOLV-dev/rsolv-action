  ...
  describe "PHP pattern AST enhancement" do
    ...
    test "PHP patterns return ast_rules in enhanced format", %{
      ...
      Enum.each(response["patterns"], fn pattern ->
        ...
        assert Map.has_key?(pattern, "contextRules"),
               "Pattern #{pattern["id"]} missing contextRules"

        assert Map.has_key?(pattern, "confidenceRules"),
               "Pattern #{pattern["id"]} missing confidenceRules"

        assert Map.has_key?(pattern, "minConfidence"),
               "Pattern #{pattern["id"]} missing minConfidence"

        # Check that it's not using the old :rules format
        refute Map.has_key?(pattern, "rules"),
               "Pattern #{pattern["id"]} has 'rules' instead of 'astRules'"

        refute Map.has_key?(pattern, ":ast_rules"),
               "Pattern #{pattern["id"]} has atom key ':ast_rules'"
      end)
    end
    ...
    test "PHP demo patterns do NOT have AST enhancement fields", %{
      ...
      Enum.each(response["patterns"], fn pattern ->
        ...
        refute Map.has_key?(pattern, "astRules"),
               "Demo pattern #{pattern_id} should NOT have astRules"

        refute Map.has_key?(pattern, "contextRules"),
               "Demo pattern #{pattern_id} should NOT have contextRules"

        refute Map.has_key?(pattern, "confidenceRules"),
               "Demo pattern #{pattern_id} should NOT have confidenceRules"

        refute Map.has_key?(pattern, "minConfidence"),
               "Demo pattern #{pattern_id} should NOT have minConfidence"
