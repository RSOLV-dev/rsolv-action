/**
 * k6 Load Test: Credential Vending API
 *
 * Target: POST /api/v1/credentials/exchange
 * Goal: 200 RPS for 5 minutes
 *
 * Tests:
 * - Ramp up to 200 RPS
 ...
 *
 * Run: k6 run scripts/load-tests/credential-vending-load-test.k6.js
 */
...
export const options = {
  stages: [
    { duration: '1m', target: 40 },   // Ramp up to 40 RPS
    { duration: '1m', target: 100 },  // Ramp up to 100 RPS
    { duration: '1m', target: 200 },  // Ramp up to 200 RPS
    { duration: '5m', target: 200 },  // Sustain 200 RPS for 5 minutes
...
// Test API keys (create these in staging before running the test)
const TEST_API_KEYS = [
...
export default function () {
  ...
  const params = {
    headers: {
      ...
      'X-API-Key': apiKey,
      'X-GitHub-Job': `loadtest-job-${__VU}-${__ITER}`,
      'X-GitHub-Run': `loadtest-run-${__VU}`,
    },
    tags: { name: 'CredentialVending' },
  };
...
export function handleSummary(data) {
  return {
    'load_tests/results/credential-vending-results.json': JSON.stringify(data, null, 2),
...
function textSummary(data, options) {
  ...
  let output = '\n';
  output += `${indent}✓ Credential Vending Load Test Results\n`;
