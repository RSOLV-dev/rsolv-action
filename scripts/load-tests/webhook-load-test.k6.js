 ...
 *
 * Run: k6 run scripts/load-tests/webhook-load-test.k6.js
 */
...
function generateEventId() {
  ...
  return `evt_load_test_${vuId}_${iteration}_${timestamp}`;
}

// Generate Stripe webhook payload
function generateWebhookPayload(eventType) {
  const eventId = generateEventId();
  const customerId = `cus_loadtest_${__VU}`;
  const subscriptionId = `sub_loadtest_${__VU}_${__ITER}`;
...
function generateSignature(payload) {
  ...
  return `t=${timestamp},v1=load_test_signature_${randomString(64)}`;
}
...
export function handleSummary(data) {
  return {
    'load_tests/results/webhook-results.json': JSON.stringify(data, null, 2),
...
function textSummary(data, options) {
  ...
  let output = '\n';
  output += `${indent}✓ Stripe Webhook Load Test Results\n`;
