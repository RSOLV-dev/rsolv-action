defmodule Rsolv.Security do
  ...
  def list_patterns_by_tier(tier_name) do
    ...
    language_patterns =
      Enum.flat_map(all_languages, fn language ->
        get_patterns_for_language(language)
      end)

    # Get framework patterns if enabled
    framework_patterns =
      if FeatureFlags.enabled?("patterns.include_framework_patterns") do
  ...
  defp get_patterns_for_language(language) do
    ...
    language_patterns =
      case Map.get(@pattern_modules, language_lower) do
        nil -> []
        module -> apply(module, :all, [])
      end
    ...
    enhanced_patterns =
      if FeatureFlags.enabled?("patterns.use_enhanced_patterns") do
