defmodule Rsolv.Security.Patterns.Javascript.HardcodedSecretApiKey do
  @moduledoc """
    ...
    %{
      ast_rules: %{
        ...
        value_analysis: %{
          ...
          # Real API keys are longer
          min_length: 16,
          # Extremely long strings unlikely to be keys
          max_length: 200,
          # High entropy suggests real keys
          entropy_check: true,
          known_prefixes: [
            # Stripe secret key
            "sk_",
      ...
      confidence_rules: %{
        ...
        base: 0.6,
        adjustments: %{
          # Matches known key patterns
          "known_api_key_format" => 0.5,
          # live_, prod_, etc.
          "production_prefix" => 0.4,
          # High randomness
          "high_entropy" => 0.3,
          # In production code
          "production_file" => 0.3,
          # Direct string assignment
          "literal_assignment" => 0.2,
          # Using env vars is safe
          "environment_variable" => -0.9,
          # Test/demo keys
          "test_key" => -0.8,
          # Too short for real key
          "short_value" => -0.5,
          # In test directory
          "test_file" => -0.7,
      ...
      # High threshold - API keys need careful filtering
      min_confidence: 0.85
    }
  end
end
