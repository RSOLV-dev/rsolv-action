defmodule Rsolv.Security.Patterns.Php.XpathInjection do
  @moduledoc """
  ...
  def pattern do
    %Pattern{
      ...
      languages: ["php"],
      regex:
        ~r/->(query|evaluate)\s*\([^)]*(?:\$_(GET|POST|REQUEST|COOKIE)|['"][^'"]*\$_(GET|POST|REQUEST|COOKIE)|['"]\s*\.\s*\$_(GET|POST|REQUEST|COOKIE))/,
      cwe_id: "CWE-643",
      owasp_category: "A03:2021",
      recommendation:
        "Validate and escape user input in XPath queries, use parameterized queries when possible",
      test_cases: %{
        vulnerable: [
      ...
      **Data Enumeration**: In applications that use XPath for data retrieval,
      ...
      cve_examples: [
        %{
          ...
          cvss: 6.5,
          note:
            "Allows remote attackers to access contents of XML documents containing display strings"
        },
        %{
          id: "CVE-2019-17591",
  ...
  def test_cases do
    %{
      ...
      negative: [
        ...
        %{
          code:
            ~S|$name = preg_replace('/[^a-zA-Z0-9]/', '', $_GET['name']); $xpath->query("//user[name='$name']");|,
  ...
  ## Examples
      ...
      iex> examples = Rsolv.Security.Patterns.Php.XpathInjection.examples()
