defmodule Rsolv.Webhooks.Handlers.GitHubHandler do
  @moduledoc """
  ...
  def extract_issue_references(text) when is_binary(text) do
    ...
    |> then(fn refs ->
      case Regex.scan(
             ~r/(?:#{keyword_pattern})\s+https?:\/\/github\.com\/([^\/]+)\/([^\/]+)\/issues\/(\d+)/i,
             text
           ) do
        matches when is_list(matches) ->
          refs ++
            Enum.map(matches, fn [_, owner, repo, num] ->
              %{
                type: :url,
                owner: owner,
                repo: repo,
  ...
  def is_rsolv_pr?(pr) do
    ...
    has_body_pattern =
      pr["body"] &&
        Enum.any?(@rsolv_identifiers.body_patterns, fn pattern ->
