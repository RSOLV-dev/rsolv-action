defmodule Rsolv.Helpers.Funnel do
  ...
  def summary(days \\ 30) do
    ...
    IO.puts("\n" <> IO.ANSI.cyan() <> "=== Conversion Funnel Summary (Last #{days} Days) ===" <> IO.ANSI.reset())
    IO.puts("")

    IO.puts("📊 Stage Counts:")
    IO.puts("  └─ Website Visits:     #{format_number(metrics.website_visits)} (#{metrics.unique_visitors} unique)")
    IO.puts("     └─ Signups:         #{format_number(metrics.signups)} (#{format_rate(metrics.visit_to_signup_rate)})")
    IO.puts("        └─ API Keys:     #{format_number(metrics.api_keys_created)} (#{format_rate(metrics.signup_to_api_key_rate)})")
    IO.puts("           └─ Activated: #{format_number(metrics.activated_users)} (#{format_rate(metrics.api_key_to_activation_rate)})")
    IO.puts("              └─ Retained: #{format_number(metrics.retained_users)} (#{format_rate(metrics.activation_to_retention_rate)})")
    IO.puts("")

    IO.puts("🎯 Overall Conversion:")
    overall_rate = if metrics.website_visits > 0 do
      Decimal.from_float(metrics.retained_users / metrics.website_visits * 100) |> Decimal.round(2)
    else
      Decimal.new("0.00")
    end
  ...
  def drop_offs(days \\ 30) do
    ...
    drop_offs = [
      ...
      {"Signup → API Key", metrics.signups - metrics.api_keys_created, metrics.signup_to_api_key_rate},
      {"API Key → Activation", metrics.api_keys_created - metrics.activated_users, metrics.api_key_to_activation_rate},
      {"Activation → Retention", metrics.activated_users - metrics.retained_users, metrics.activation_to_retention_rate}
    ]
    ...
    |> Enum.each(fn {{stage, lost, conversion_rate}, index} ->
      ...
      IO.puts("#{index}. #{stage}")
      IO.puts("   Lost: #{lost} users (#{format_rate(Decimal.new(100) |> Decimal.sub(conversion_rate))} drop-off)")
  ...
  def recent_journeys(limit \\ 10) do
    ...
    |> Enum.each(fn journey ->
      ...
      IO.puts("  #{if journey.completed_api_key, do: "✅", else: "⏳"} API Key: #{format_datetime(journey.api_key_created_at)}")
      IO.puts("  #{if journey.completed_activation, do: "✅", else: "⏳"} Activated: #{format_datetime(journey.first_api_call_at)}")
      IO.puts("  #{if journey.completed_retention, do: "✅", else: "⏳"} Retained: #{format_datetime(journey.second_api_call_at)}")

      if journey.utm_source do
        IO.puts("  📍 Source: #{journey.utm_source}" <> if journey.utm_campaign, do: " / #{journey.utm_campaign}", else: "")
      end
  ...
  def not_created_api_key(limit \\ 10) do
    journeys = FunnelTracking.list_journeys(
      limit: limit
    )
    ...
    if Enum.empty?(journeys) do
      ...
      journeys
      |> Enum.each(fn journey ->
        days_ago = if journey.signup_at do
  ...
  def not_activated(limit \\ 10) do
    ...
    if Enum.empty?(journeys) do
      ...
      journeys
      |> Enum.each(fn journey ->
        days_ago = if journey.api_key_created_at do
  ...
  def not_retained(limit \\ 10) do
    ...
    if Enum.empty?(journeys) do
      ...
      journeys
      |> Enum.each(fn journey ->
        days_ago = if journey.first_api_call_at do
  ...
  def daily(start_date, end_date \\ Date.utc_today()) do
    ...
    |> Enum.each(fn metrics ->
      IO.puts("#{metrics.period_start} | #{pad(metrics.website_visits, 6)} | #{pad(metrics.signups, 7)} | #{pad(metrics.api_keys_created, 8)} | #{pad(metrics.activated_users, 9)} | #{pad(metrics.retained_users, 8)}")
    end)
