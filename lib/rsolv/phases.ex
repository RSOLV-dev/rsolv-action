defmodule Rsolv.Phases do
  ...
  def retrieve(repo_string, issue_number, commit_sha, %ApiKey{} = api_key) do
    with {:ok, customer} <- get_customer_from_api_key(api_key),
      ...
      if is_nil(repository) do
        ...
        phase_data =
          case get_scan_execution(repository.id, commit_sha) do
            nil -> phase_data
        ...
        phase_data =
          case get_validation_execution(repository.id, issue_number) do
            nil ->
              phase_data
        ...
        phase_data =
          case get_mitigation_execution(repository.id, issue_number) do
            nil ->
              phase_data
  ...
  defp get_repository(repo_attrs) do
    case Repo.get_by(Repository,
      ...
      # No repository means no data to retrieve
      nil -> {:ok, nil}
      repo -> {:ok, repo}
    end
  end
