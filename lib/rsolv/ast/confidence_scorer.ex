defmodule Rsolv.AST.ConfidenceScorer do
  ...
  defp get_base_confidence(pattern_type) do
    case pattern_type do
      :code_injection -> 0.8
      # Command injection is serious
      :command_injection -> 0.8
      :remote_code_execution -> 0.85
      # RCE is critical (eval patterns)
      :rce -> 0.85
      :sql_injection -> 0.75
      :xss -> 0.7
  ...
  defp adjust_for_framework_protection(confidence, %{framework_protection: true}),
    do: confidence * 0.4
  ...
  defp adjust_for_severity(confidence, context) do
    severity_multiplier =
      case {context[:pattern_type], context[:function_name], context[:in_database_call]} do
  ...
  defp analyze_factors(context, language) do
    ...
    factors =
      if context[:framework_protection] do
    ...
    factors =
      if is_binary(context[:file_path]) && String.contains?(context[:file_path], ["test", "spec"]) do
    ...
    factors =
      if language == "php" && context[:pattern_type] == :sql_injection do
