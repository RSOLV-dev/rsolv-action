defmodule Rsolv.AST.FallbackStrategy do
  @moduledoc """
  ...
  alias Rsolv.AST.{ParserRegistry, ASTNormalizer}
  ...
  defmodule AnalysisResult do
    ...
    defstruct [
      ...
      # boolean
      :ast_available,
      # normalized AST if available
      :ast,
      # standardized error if AST failed
      :error,
  ...
  defmodule FallbackAnalysis do
    ...
    defstruct [
      ...
      :confidence,
      # whether partial AST was recovered
      :partial_ast_available,
  ...
  def analyze_with_fallback(session_id, customer_id, language, code, options \\ %{}) do
    ...
    case check_cache(cache_key) do
      {:ok, cached_result} ->
        ...
        updated_timing =
          Map.merge(cached_result.timing, %{
            cache_hit: true,
            total_ms: total_time
          })
  ...
  defp estimate_complexity(code) do
    ...
    factors = [
      # Conditionals
      {~r/if\s*\(|elif\s+|else\s*{/, 1},
      # Loops
      {~r/for\s*\(|while\s*\(|\.forEach/, 2},
      # Error handling
      {~r/try\s*{|catch\s*\(/, 2},
      # Functions
      {~r/function|def\s+|func\s+/, 1},
      # Classes
      {~r/class\s+\w+/, 3},
      # Logical operators
      {~r/\|\||&&/, 1},
  ...
  defp calculate_metrics(code) do
    ...
    lines =
      if List.last(lines) == "" and length(lines) > 1 do
  ...
  defp detect_weak_crypto(code, _language) do
    ...
    weak_key_results =
      Regex.scan(~r/randomBytes\s*\(\s*(\d+)\s*\)/, code, capture: :all_but_first)
    ...
    weak_key_results ++
      find_patterns(
        code,
        Enum.reject(patterns, fn {p, _, _} -> p == ~r/randomBytes\s*\(\s*(\d+)\s*\)/ end),
        "weak_cryptography"
      )
  end
  ...
  defp generate_recommendations(patterns, _language) do
    ...
    recommendations =
      if grouped["command_injection"] do
        [
          "Sanitize user input and avoid shell execution with user-controlled data"
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if Enum.any?(patterns, fn p ->
           p.description =~ "exec" or
             (p.type == "command_injection" and
                patterns |> Enum.any?(fn pp -> pp.description =~ "exec" end))
         end) do
    ...
    recommendations =
      if grouped["weak_cryptography"] do
        [
          "Use strong cryptographic algorithms and adequate key sizes (>= 128 bits)"
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if grouped["hardcoded_secrets"] do
  ...
  defp balanced_brackets?(code) do
    ...
    {_, balanced} =
      Enum.reduce(chars, {[], true}, fn char, {stack, balanced} ->
        cond do
  ...
  defp detect_warnings(code, metrics) do
    ...
    warnings =
      if String.contains?(code, String.duplicate("(", 100)) do
