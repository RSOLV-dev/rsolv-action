defmodule Rsolv.AST.CodeRetention do
  @moduledoc """
  ...
  def verify_no_code_in_processes(code) when is_binary(code) do
    ...
    found_in =
      Enum.reduce(processes, [], fn pid, acc ->
  ...
  def verify_findings_scrubbed(findings) when is_list(findings) do
    code_leaks =
      Enum.flat_map(findings, fn finding ->
        ...
        leaked =
          if description && contains_code_pattern?(description) do
        ...
        leaked =
          if context && contains_code_pattern?(inspect(context)) do
  ...
  def generate_retention_report do
    ...
    ets_issues =
      Enum.flat_map(ets_tables, fn table ->
        ...
        _ ->
          ...
          case Process.info(pid, :initial_call) do
            {:initial_call, {module, _, _}} ->
              ...
              if String.starts_with?(module_name, "Rsolv.AST") do
                # Check process dictionary
                case Process.info(pid, :dictionary) do
                  {:dictionary, dict} ->
                    Enum.any?(dict, fn {_key, value} ->
      ...
      case :ets.info(table, :name) do
        ...
        name when is_atom(name) ->
          ...
          name not in [
            ...
            :ac_tab,
            :file_io_servers,
            # Expected to contain ASTs temporarily
            :ast_cache,
            # Contains pattern definitions
            :security_patterns,
            # Contains compiled patterns
            :pattern_cache
          ]
  ...
  defp find_code_in_ast(ast) when is_map(ast) do
    ...
    issues =
      if Map.has_key?(ast, "value") && is_binary(ast["value"]) && String.length(ast["value"]) > 10 do
    ...
    issues =
      if Map.has_key?(ast, "children") && is_list(ast["children"]) do
