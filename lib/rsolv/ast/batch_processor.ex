defmodule Rsolv.AST.BatchProcessor do
  ...
  def process_batch(files, opts \\ []) do
    ...
    max_analysis_concurrency =
      Keyword.get(opts, :max_analysis_concurrency, @default_max_concurrency)
    ...
    successful_parses =
      Enum.filter(parse_results, fn
        {:ok, _} -> true
        _ -> false
      end)

    files_with_context =
      Enum.map(successful_parses, fn {:ok, file_data} ->
        {file_data, %{path: file_data.path, language: file_data.language}}
      end)

    {analysis_time, analysis_results} =
      :timer.tc(fn ->
    ...
    analysis_lookup =
      Map.new(analysis_results, fn {context, findings} ->
          ...
          {:ok, _file_data} ->
            ...
            %BatchResult{
              ...
              metrics: %BatchMetrics{
                ...
                # Since we do parsing in analysis phase
                parse_time_ms: div(analysis_time, 1000),
