defmodule Rsolv.AST.PortWorker do
  @moduledoc """
  ...
  def init(config) do
    ...
    sandbox_config =
      Sandbox.create_beam_sandbox_config(
        config.language,
        %{
    ...
    case start_sandboxed_port(config, sandbox_config) do
      {:ok, port, resource_tracker} ->
        ...
        state =
          if config[:max_heap_size] do
            # Check every second
            timer = Process.send_after(self(), :memory_check, 1000)
            # Reuse timer field for simplicity
            %{state | health_check_timer: timer}
          else
            state
          end
  ...
  defp parse_messages(buffer) do
    ...
    case lines do
      ...
      lines ->
        ...
        messages =
          Enum.flat_map(complete_lines, fn line ->
      ...
      %{"id" => request_id} = response ->
        ...
        case Map.pop(state.requests, request_id) do
          ...
          {from, requests} ->
            ...
            reply =
              case response do
                # Return full response with error
                %{"error" => _error} -> {:ok, response}
