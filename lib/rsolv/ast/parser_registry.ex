defmodule Rsolv.AST.ParserRegistry do
  ...
  def parse_code(session_id, customer_id, language, code) do
    GenServer.call(
      __MODULE__,
      {:parse_code, session_id, customer_id, language, code},
      @default_timeout
    )
  end
        ...
        case Map.get(state.parsers, language) do
          nil ->
            ...
            {:error, standardized_error} =
              ASTErrorHandler.standardize_error(
          ...
          parser_config ->
            ...
            {parser_id, updated_state} =
              get_or_create_parser(session_id, language, parser_config, state)

            # Parse the code
            start_time = System.monotonic_time(:millisecond)

            {reply, updated_stats} =
              case parse_with_parser(parser_id, code, parser_config) do
                {:ok, ast} ->
                ...
                {:error, raw_error} ->
                  ...
                  standardized_error =
                    case standardize_parser_error(raw_error, language, code) do
                      {:error, error} -> error
  ...
  def handle_call(:get_statistics, _from, state) do
    success_rate =
      if state.stats.total_requests > 0 do
  ...
  defp parse_with_parser(parser_id, code, parser_config) do
    ...
    case code do
      ...
      _ ->
        case PortSupervisor.call_port(
               nil,
               parser_id,
               code,
               parser_config.timeout || @default_timeout
             ) do
          {:ok, response} ->
