defmodule Rsolv.AST.SessionManager do
  ...
  defp add_session_to_ets(session) do
    ...
    customer_sessions =
      case :ets.lookup(@customer_sessions_table, session.customer_id) do
  ...
  defp remove_session_from_ets(session_id) do
    case get_session_from_ets(session_id) do
      ...
      other ->
        ...
        Logger.warning(
          "Unexpected data type in session table for #{session_id}: #{inspect(other)}"
        )
  ...
  defp cleanup_sessions_ets do
    expired_sessions =
      try do
        ...
        # Table might not exist or have invalid data
        _ -> []
      end
    ...
    try do
      ...
      Enum.each(languages, fn language ->
        ...
        case GenServer.call(
               ...
               {:cleanup_session_parser, session_id, language},
               5000
             ) do
          :ok -> Logger.debug("Cleaned up #{language} parser for session #{session_id}")
          # Parser didn't exist or already cleaned up
          _ -> :ok
        end
      end)
    rescue
