defmodule Rsolv.AST.PortSupervisor do
  @moduledoc """
  ...
  def start_port(supervisor, config) do
    ...
    if pooled_port_id do
      ...
      restart_type =
        if config[:max_restarts] && config[:max_restarts] > 0 do
          :permanent
        else
          :transient
        end
      ...
      case DynamicSupervisor.start_child(supervisor, spec) do
        {:ok, pid} ->
          ...
          safe_ets_insert(
            :port_registry,
            {port_id, pid, config, System.monotonic_time(:millisecond)}
          )
      ...
      [{^port_id, pid, config, _started_at}] ->
        ...
        case DynamicSupervisor.start_child(supervisor, spec) do
          {:ok, new_pid} ->
            ...
            safe_ets_insert(
              :port_registry,
              {port_id, new_pid, config, System.monotonic_time(:millisecond)}
            )
  ...
  defp add_to_pool(language, port_id) do
    ...
    case :ets.lookup(:port_pools, language) do
      [{^language, pool}] ->
        new_pool =
          if length(pool) >= max_pool_size do
            # Remove oldest
  ...
  defp update_stats(port_id, type) do
    case :ets.lookup(:port_stats, port_id) do
      [{^port_id, stats}] ->
        ...
        # Return restart count for debugging
        updated_stats.restarts
