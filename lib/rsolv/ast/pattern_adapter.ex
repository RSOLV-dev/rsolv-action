defmodule Rsolv.AST.PatternAdapter do
  @moduledoc """
  ...
  alias Rsolv.Security.{Pattern, ASTPattern, PatternRegistry, PatternServer}
  require Logger
  ...
  defp convert_ast_rules_to_pattern(%{node_type: node_type} = rules) do
    ...
    base_pattern =
      if node_type == "BinOp" && rules[:op] do
    ...
    base_pattern =
      if parent = rules[:parent_node] do
    ...
    base_pattern =
      if context = rules[:context_analysis] do
        base_pattern
    ...
    base_pattern =
      if sql_context = rules[:sql_context] do
        base_pattern
    ...
    base_pattern =
      if ancestors = rules[:ancestor_requirements] do
    ...
    base_pattern =
      if left_side = rules[:left_side] do
    ...
    base_pattern =
      if right_side = rules[:right_side_analysis] do
    ...
    base_pattern =
      if identifier_check = rules[:identifier_check] do
    ...
    base_pattern =
      if value_types = rules[:value_types] do
    ...
    base_pattern =
      ...
      # For eval pattern with alternatives
      |> maybe_add_rule(:callee, rules)
      # For command injection patterns
      |> maybe_add_rule(:callee_names, rules)
      # For Math.random, etc.
      |> maybe_add_rule(:callee_pattern, rules)
      # For context-based checks
      |> maybe_add_rule(:usage_analysis, rules)
      |> maybe_add_rule(:argument_contains, rules)
      # For argument checking
      |> maybe_add_rule(:argument_analysis, rules)
  ...
  def enhance_pattern(%Pattern{} = pattern) do
    ...
    enhanced =
      try do
        ...
        module =
          Module.concat(
            (["Rsolv", "Security", "Patterns"] ++ String.split(module_name, "."))
            |> Enum.drop(3)
          )
    ...
    enhanced ||
      case ASTPattern.enhance(pattern) do
        %ASTPattern{} = enhanced ->
          enhanced
  ...
  defp do_load_patterns(language) do
    ...
    language_patterns =
      case Process.whereis(PatternServer) do
        nil ->
    ...
    Logger.info(
      "PatternAdapter loading patterns for #{language}: found #{length(language_patterns)} patterns"
    )
  ...
  defp pattern_id_to_module_name(pattern_id) do
    # Convert pattern ID to module name
    # "python-sql-injection-concat" -> Rsolv.Security.Patterns.Python.SqlInjectionConcat
    ...
    "Rsolv.Security.Patterns.#{language}.#{pattern_name}"
  end
end
