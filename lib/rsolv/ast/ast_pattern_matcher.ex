defmodule Rsolv.AST.ASTPatternMatcher do
  ...
  defp traverse_and_match(ast, pattern, full_pattern, language, path, context) when is_map(ast) do
    ...
    node_matches =
      if pattern_matches && context_passes do
  ...
  defp traverse_and_match(ast, pattern, full_pattern, language, path, context)
       when is_list(ast) do
    ast
    |> Enum.with_index()
  ...
  defp passes_context_requirements?(node, pattern, _full_pattern, context) do
    ...
    parent_ok =
      if parent_req = pattern["_parent_requirements"] do
    ...
    sql_ok =
      if pattern["_contains_sql_keywords"] || pattern["_contains_sql"] do
    ...
    user_input_ok =
      if pattern["_has_user_input_in_concatenation"] || pattern["_has_user_input"] do
    ...
    method_ok =
      if methods = pattern["_method_names"] do
    ...
    callee_ok =
      if callee_names = pattern["_callee_names"] do
    ...
    callee_pattern_ok =
      if callee_pattern = pattern["_callee_pattern"] do
    ...
    callee_object_property_ok =
      if pattern["_callee_object"] && pattern["_callee_property"] do
    ...
    usage_ok =
      if usage_analysis = pattern["_usage_analysis"] do
    ...
    identifier_ok =
      if identifier_check = pattern["_identifier_check"] do
    ...
    value_ok =
      if value_analysis = pattern["_value_analysis"] do
    ...
    value_type_ok =
      if value_types = pattern["_value_types"] do
    ...
    left_side_ok =
      if left_req = pattern["_left_side"] do
    ...
    arg_analysis_ok =
      if arg_req = pattern["_argument_analysis"] do
    ...
    sql_pattern_ok =
      if pattern["_contains_sql_pattern"] do
        contains_sql_keywords?(node, context)
      else
        true
      end
    ...
    string_ok =
      if pattern["_left_or_right_is_string"] do
    ...
    parent_ok && sql_ok && user_input_ok && db_call_ok && method_ok && callee_ok &&
      callee_pattern_ok && callee_object_property_ok && usage_ok && identifier_ok && value_ok &&
      value_type_ok &&
  ...
  defp check_parent_requirements(context, parent_req) do
    ...
    # Check parent type
    type_ok =
      if req_type = parent_req["type"] || parent_req[:type] do
    ...
    callee_ok =
      if parent_req["callee_matches"] || parent_req[:callee_matches] do
  ...
  # Older Python AST format
  defp is_string_node?(%{"type" => "Str", "s" => _}), do: true
  ...
  defp extract_method_name(%{"type" => "MemberExpression", "property" => %{"name" => name}}),
    do: name
  ...
  defp extract_callee_name(%{"type" => "MemberExpression", "property" => %{"name" => name}}),
    do: name
  ...
  defp check_left_side_requirements(node, left_req) do
    ...
    if left == nil do
      ...
      type_ok =
        if req_type = left_req["object_type"] || left_req[:object_type] do
      ...
      property_ok =
        if req_prop = left_req["property"] || left_req[:property] do
          case left do
  ...
  def check_argument_analysis(node, arg_req, _context \\ %{}) do
    ...
    position_value_ok =
      if position = arg_req["position"] || arg_req[:position] do
        ...
        case Enum.at(args, position) do
          ...
          when type in ["Literal", "StringLiteral"] and is_binary(value) ->
          ...
          %{"type" => "TemplateLiteral", "quasis" => [%{"value" => %{"raw" => raw}} | _]}
          when is_binary(raw) ->
          ...
          _ ->
            # Argument exists but isn't a string literal we can check
            false
        end
      else
    ...
    if not position_value_ok do
      ...
      if arg_req["contains_sensitive_keywords"] || arg_req[:contains_sensitive_keywords] do
        ...
        first_arg_ok =
          if arg_req["first_arg_contains_user_input"] || arg_req[:first_arg_contains_user_input] do
        ...
        string_type_ok =
          if arg_req["is_string_type"] || arg_req[:is_string_type] do
            case Enum.at(args, 0) do
              %{"type" => type}
              when type in ["Literal", "StringLiteral", "TemplateLiteral", "Identifier"] ->
                true
        ...
        not_static_ok =
          if arg_req["not_static_string"] || arg_req[:not_static_string] do
            case Enum.at(args, 0) do
  ...
  defp build_match(node, pattern, _path, context) do
    %{
      ...
      type: pattern.id,
      # Get pattern_type from the converted format
      ...
      # Pass through min_confidence
      min_confidence: Map.get(pattern, :min_confidence, 0.7),
      context: build_context(node, context),
      recommendation:
        Map.get(pattern, :recommendation, "Review this code for potential security issues")
    }
  end
  ...
  defp calculate_confidence(node, pattern, context) do
    ...
    if map_size(confidence_rules) > 0 do
      ...
      cond do
        ...
        # Better default for crypto patterns
        String.contains?(pattern.id, "crypto") -> 0.7
        true -> 0.7
      end
    end
  end
  ...
  defp build_context(node, context) do
    ...
    base =
      if function_name = Map.get(context, :in_function) do
  ...
  defp update_context(context, node, key) do
    ...
    new_context =
      if is_map(node) && Map.has_key?(node, "type") do
  ...
  defp maybe_add_function_context(context, %{
  ...
  defp extract_string_value(%{
         "type" => "TemplateLiteral",
         "quasis" => [%{"value" => %{"raw" => v}} | _]
       }),
       do: v
  ...
  defp check_usage_analysis(_node, analysis, context) do
    ...
    checks =
      if get_option(analysis, :check_variable_name) do
        ...
        var_check =
          case parent do
            %{"type" => "VariableDeclarator", "id" => %{"name" => name}} ->
              ...
              security_patterns =
                ~r/token|key|secret|password|auth|session|nonce|salt|iv|uuid|guid/i
    ...
    checks =
      if get_option(analysis, :check_context) do
        # Check if we're in a security-related context
        in_security_context =
          Map.get(context, :in_security_function, false) ||
    ...
    checks =
      if get_option(analysis, :check_transformations) do
        parent = Map.get(context, :parent_node)
