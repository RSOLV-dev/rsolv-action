defmodule Rsolv.AST.ASTCache do
  @moduledoc """
    ...
    case Map.get(state.entries, cache_key) do
      ...
      entry ->
        ...
        if expired?(entry, now, state.config.ttl_seconds, state.config.refresh_ttl_on_access) do
          ...
          # Cache hit
          updated_entry =
            if state.config.refresh_ttl_on_access do
              %{entry | last_accessed_at: now, access_count: (entry.access_count || 0) + 1}
            else
  ...
  def handle_call({:invalidate_by_language, language}, _from, state) do
    {entries_to_remove, remaining_entries} =
      Enum.split_with(state.entries, fn
    ...
    removed_memory =
      Enum.reduce(entries_to_remove, 0, fn {_key, entry}, acc ->
    ...
    updated_state = %{state | entries: Map.new(remaining_entries), stats: updated_stats}
  ...
  def handle_call({:warm_cache, files, ast_generator_fn}, _from, state) do
    {warmed_count, updated_state} =
      Enum.reduce(files, {0, state}, fn {file_or_hash, language}, {count, acc_state} ->
  ...
  def handle_info(:cleanup_expired, state) do
    ...
    expired_memory =
      Enum.reduce(expired_entries, 0, fn {_key, entry}, acc ->
        acc + (entry.size_bytes || 0)
      end)
    ...
    updated_state = %{state | entries: Map.new(valid_entries), stats: updated_stats}
  ...
  defp do_put(state, file_hash, ast, language) do
    ...
    size_bytes =
      if state.config.enable_memory_tracking do
        estimate_size(ast)
      else
        0
      end
  ...
  defp evict_lru_entries(state, count_to_evict) do
    if count_to_evict <= 0 do
      ...
      evicted_memory =
        Enum.reduce(sorted_entries, 0, fn {_key, entry}, acc ->
          acc + (entry.size_bytes || 0)
        end)
  ...
  defp evict_until_memory_limit(state, target_memory) do
    if state.stats.memory_usage_bytes <= target_memory do
      ...
      {to_evict, _to_keep} =
        Enum.reduce_while(sorted_entries, {[], state.stats.memory_usage_bytes}, fn {key, entry},
                                                                                   {evict_acc,
                                                                                    current_memory} ->
      ...
      evicted_memory =
        Enum.reduce(to_evict, 0, fn key, acc ->
  ...
  defp expired?(entry, now, ttl_seconds, refresh_ttl_on_access) do
    ...
    expiry_time =
      if refresh_ttl_on_access && entry.last_accessed_at do
