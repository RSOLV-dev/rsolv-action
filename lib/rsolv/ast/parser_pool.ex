defmodule Rsolv.AST.ParserPool do
  @moduledoc """
  ...
  alias Rsolv.AST.{PortSupervisor, ParserRegistry}
  ...
  def start_link(config) do
    {name, config} =
      case config do
        %{name: name} ->
          {name, Map.delete(config, :name)}

        config when is_list(config) ->
          {Keyword.get(config, :name, __MODULE__), Keyword.delete(config, :name)}
  ...
  def init(config) do
    ...
    state =
      Enum.reduce(state.languages, state, fn language, acc ->
  ...
  def handle_call(:get_pool_status, _from, state) do
    status =
      Enum.reduce(state.pools, %{}, fn {language, parsers}, acc ->
        counts =
          Enum.reduce(parsers, %{available: 0, busy: 0, warming: 0}, fn {_id, info}, counts ->
            case info.status do
  ...
  def handle_call({:get_parser_status, language}, _from, state) do
    ...
    status =
      Enum.reduce(parsers, %{}, fn {id, info}, acc ->
        Map.put(acc, id, %{
          warmed: info.warmed,
          status: info.status,
  ...
  def handle_call(:get_metrics, _from, state) do
    metrics =
      Enum.reduce(state.metrics, %{}, fn {language, lang_metrics}, acc ->
  ...
  def handle_cast({:report_success, language, parser_id, metrics}, state) do
    ...
    state = %{
      state
      | pools:
          Map.update(state.pools, language, %{}, fn lang_pools ->
  ...
  defp initialize_language_pool(state, language) do
    ...
    %{
      ...
      | pools: Map.put(state.pools, language, parsers),
        metrics:
          Map.put(state.metrics, language, %{
            checkouts: 0,
            checkins: 0,
  ...
  defp spawn_parser(state, language) do
    ...
    if parser_id do
      ...
      %{
        state
        | pools:
            Map.update(state.pools, language, %{parser_id => info}, fn lang_pools ->
  ...
  defp remove_parser(state, language, parser_id) do
    %{state | pools: Map.update(state.pools, language, %{}, &Map.delete(&1, parser_id))}
  end
  ...
  defp record_checkout(state, language, parser_id) do
    state = %{
      state
      | metrics:
          Map.update(state.metrics, language, %{checkouts: 1}, fn lang_metrics ->
  ...
  defp record_checkin(state, language, _parser_id) do
    %{
      state
      | metrics:
          Map.update(state.metrics, language, %{checkins: 1}, fn lang_metrics ->
  ...
  defp calculate_metrics(lang_metrics, parsers) do
    ...
    total_successful =
      Enum.reduce(parsers, 0, fn {_id, info}, acc ->
    ...
    health_score =
      if total_successful + total_failed > 0 do
    ...
    %{
      ...
      # TODO: Implement wait time tracking
      avg_wait_time_ms: 0,
  ...
  defp should_scale_down?(state, language) do
    ...
    # 5 minutes default
    scale_down_after = state.config[:scale_down_after_ms] || 300_000
    ...
    case lru_parser do
      {parser_id, _info} ->
        ...
        try do
          ...
          # Ignore errors - parser might already be dead
          _ -> :ok
        end
