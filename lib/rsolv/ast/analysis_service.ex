defmodule Rsolv.AST.AnalysisService do
  @moduledoc """
  ...
  defp do_analyze_file(file, options) do
    ...
    case get_or_parse_ast(file) do
      {:ok, ast} ->
        ...
        findings =
          if options["includeSecurityPatterns"] == false do
            []
          else
  ...
  defp parse_file(file, cache_key) do
    ...
    cond do
      ...
      true ->
        ...
        case ParserRegistry.parse_code(
               session.id,
               "analysis-service",
               file.language,
               file.content
  ...
  defp detect_patterns(file, ast, options) do
    ...
    code_context =
      ContextAnalyzer.analyze_code(file.content, file.language, %{
        path: file.path
      })
    ...
    if path_context.should_skip do
      ...
      |> Enum.map(fn match ->
        ...
        confidence =
          if match_confidence && match_confidence > 0 do
        ...
        Logger.debug(
          "Match confidence: #{confidence}, min_confidence: #{Map.get(match, :min_confidence, 0.7)}, match: #{inspect(match)}"
        )
  ...
  defp convert_pattern_type(type) when is_binary(type) do
    ...
    cond do
      ...
      # eval patterns are RCE (Remote Code Execution)
      String.contains?(type, "eval") -> :rce
  ...
  defp build_finding_from_match(match, confidence, path_context, code_context) do
    %Finding{
      ...
      # AST patterns don't have CWE/OWASP yet
      references: %{}
    }
  end
