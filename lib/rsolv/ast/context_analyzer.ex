defmodule Rsolv.AST.ContextAnalyzer do
  @moduledoc """
  ...
  defp do_analyze_path(path, options) do
    ...
    confidence_multiplier =
      cond do
        # Even lower in strict mode
        is_test && strict_mode -> 0.1
        is_test -> 0.3
        # Lower in strict mode
        is_example && strict_mode -> 0.3
        is_example -> 0.5
        is_vendor -> 0.0
        true -> 1.0
      end
  ...
  defp detect_framework(code, language) do
    case language do
      ...
      "javascript" ->
        cond do
          code =~ ~r/require\s*\(\s*['"]express['"]/ || code =~ ~r/from\s+['"]express['"]/ ->
            "express"

          code =~ ~r/require\s*\(\s*['"]koa['"]/ || code =~ ~r/from\s+['"]koa['"]/ ->
            "koa"

          code =~ ~r/require\s*\(\s*['"]fastify['"]/ || code =~ ~r/from\s+['"]fastify['"]/ ->
            "fastify"
  ...
  defp uses_safe_patterns?(code, _language, pattern_type) do
    case pattern_type do
      :sql_injection ->
        ...
        # Rails style
        # Placeholder style (with optional quote)
        # Python style
        # Named parameters
        code =~ ~r/where\s*\(\s*\w+:\s*\w+/ ||
          code =~ ~r/\?['"]?\s*[,)]/ ||
          code =~ ~r/%s/ ||
          code =~ ~r/:\w+/
  ...
  defp calculate_safety_score(factors) do
    ...
    score =
      case factors.input_handling do
