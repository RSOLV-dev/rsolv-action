defmodule Mix.Tasks.CreateStagingKey do
  ...
  def run(_) do
    ...
    api_key = "staging_phase_test_#{timestamp}_" <> Base.encode16(:crypto.strong_rand_bytes(16), case: :lower)
    ...
    {:ok, _} = Repo.query(
      ...
      """,
      [api_key, "Staging Phase Test Key", customer_id, ["all"], true, DateTime.utc_now(), DateTime.utc_now()]
    )
    ...
    {:ok, _} = Repo.query(
      ...
      """,
      [customer_id, "github", "RSOLV-dev", DateTime.utc_now(), Jason.encode!(metadata), DateTime.utc_now(), DateTime.utc_now()]
    )
    ...
    {:ok, _} = Repo.query(
      ...
      VALUES ($1, $2, $3, $4, $5::jsonb, $6, $7)
      """,
      [customer_id, "github", "test-org", DateTime.utc_now(), Jason.encode!(metadata), DateTime.utc_now(), DateTime.utc_now()]
    )
