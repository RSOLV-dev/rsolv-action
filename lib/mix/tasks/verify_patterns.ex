defmodule Mix.Tasks.VerifyPatterns do
    ...
    counts = Enum.map(pattern_modules, fn {module, name} ->
      try do
        ...
        pattern_functions = Enum.filter(functions, fn {func_name, arity} ->
          arity == 0 and func_name != :__info__ and not String.starts_with?(Atom.to_string(func_name), "_")
        end)
  ...
  defp analyze_languages do
    ...
    languages = ["JavaScript/TypeScript", "Python", "Ruby", "Java", "PHP", "Elixir/Phoenix", "CVE (Cross-language)"]
  ...
  defp validate_structure do
    ...
    validations = Enum.map(sample_patterns, fn {name, pattern} ->
      ...
      has_test_cases = Map.has_key?(pattern, :test_cases) and 
  ...
  defp test_detection do
    ...
    test_results = Enum.map(sample_patterns, fn {name, pattern} ->
      ...
      if all_passed do
        ...
        IO.puts("  #{if all_passed, do: "✅", else: "❌"} #{name} - Vulnerable: #{v_status}#{vulnerable_tests.detected}/#{vulnerable_tests.total}, Safe: #{s_status}#{safe_tests.not_detected}/#{safe_tests.total}")
      end
  ...
  defp test_integration do
    ...
    try do
      ...
      safe_code = ~s|const query = "SELECT * FROM users WHERE id = ?"; db.execute(query, [userId]);|
      ...
      if integration_working do
        ...
        IO.puts("  ❌ Integration test failed - Vulnerable: #{vulnerable_detected}, Safe: #{safe_detected}")
      end
  ...
  defp print_summary(summary) do
    ...
    IO.puts("   - #{summary.total_categories} total categories (#{summary.total_languages} languages + #{summary.total_frameworks} frameworks)")
    IO.puts("   - Pattern detection #{if summary.detection_accuracy_rate > 80, do: "working well", else: "needs improvement"}")
    IO.puts("   - System integration #{if summary.integration_test_passed, do: "functional", else: "has issues"}")
  end
