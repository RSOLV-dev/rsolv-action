defmodule RsolvApi.Security.Patterns.Django.DebugSettings do
  @moduledoc """
  ...
  def vulnerability_metadata do
    %{
      ...
      likelihood: "High - DEBUG = True is commonly left enabled during rushed deployments or inadequate production configuration",
  ...
  def ast_enhancement do
    %{
      ...
      context_rules: %{
        ...
        safe_defaults: [
          ~r/['"]False['"]/, ~r/False\b/, ~r/0\b/
        ],
        
        # Conditional patterns that might be safe
        conditional_patterns: [
          ~r/if\s+.*runserver/, ~r/if\s+.*development/,
          ~r/if\s+.*local/, ~r/if\s+.*debug/i
        ]
      },
  ...
  def applies_to_file?(file_path, frameworks \\ nil) do
    ...
    is_settings_file = String.contains?(file_path, "settings") ||
