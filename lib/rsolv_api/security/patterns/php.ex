defmodule RsolvApi.Security.Patterns.Php do
  ...
  def sql_injection_concat do
    %Pattern{
      ...
      severity: :critical,
      languages: ["php"],
      regex: ~r/\$\w+\s*=\s*["']SELECT.*WHERE.*["']\s*\.\s*\$_(GET|POST|REQUEST|COOKIE)/,
...
$stmt->bind_param("s", $_POST['author']);|
  ...
  def sql_injection_interpolation do
    %Pattern{
      ...
      severity: :critical,
      languages: ["php"],
      regex: ~r/["']SELECT.*WHERE.*\$_(GET|POST|REQUEST|COOKIE)\[/,
      default_tier: :protected,
      cwe_id: "CWE-89",
      owasp_category: "A03:2021",
...
$stmt->execute(['name' => $_GET['name']]);|
  ...
  def command_injection do
    %Pattern{
      ...
      severity: :critical,
      languages: ["php"],
      regex: ~r/(system|exec|shell_exec|passthru|`[^`]*`)\s*\(\s*.*\$_(GET|POST|REQUEST|COOKIE)/,
      default_tier: :protected,
      cwe_id: "CWE-78",
      owasp_category: "A03:2021",
...
$files = scandir($directory);|
  ...
  def xss_echo do
    %Pattern{
      ...
      severity: :high,
      languages: ["php"],
      regex: ~r/echo\s+.*\$_(GET|POST|REQUEST|COOKIE)(?!\s*,\s*ENT_QUOTES)/,
      default_tier: :public,
      cwe_id: "CWE-79",
      owasp_category: "A03:2021",
...
$xml = simplexml_load_string($_POST['xml'], 'SimpleXMLElement', LIBXML_NOCDATA);|
  ...
  def path_traversal do
    %Pattern{
      ...
      severity: :high,
      languages: ["php"],
      regex: ~r/(file_get_contents|fopen|include|require)\s*\(\s*['"]?[^'"]*\$_(GET|POST|REQUEST)/,
      default_tier: :protected,
      cwe_id: "CWE-22",
      owasp_category: "A01:2021",
...
ldap_search($ds, $dn, "(uid=$username)");|
  ...
  def xpath_injection do
    %Pattern{
      ...
      severity: :high,
      languages: ["php"],
      regex: ~r/->query\s*\(\s*["'][^"']*\$_(GET|POST|REQUEST)/,
      default_tier: :protected,
      cwe_id: "CWE-643",
      owasp_category: "A03:2021",
