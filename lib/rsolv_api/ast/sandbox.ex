defmodule RsolvApi.AST.Sandbox do
  @moduledoc """
  ...
  @default_limits %{
    max_heap_size: 64_000_000,   # ~256MB max heap
    timeout_ms: 30_000,          # 30 seconds
    max_reductions: 2_000_000,   # Limit CPU time per operation
    max_ports: 1,                # Only allow one port per parser
    max_ets_tables: 5            # Limit ETS table creation
  }
  ...
  def get_resource_usage(pid, resource_tracker) do
    case Process.info(pid, [:memory, :total_heap_size, :reductions, :message_queue_len]) do
      ...
      info ->
        ...
        tracked_stats = case :ets.lookup(resource_tracker, :stats) do
          [{:stats, stats}] -> stats
          [] -> %{}
        end
