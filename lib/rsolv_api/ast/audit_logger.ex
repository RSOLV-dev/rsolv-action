defmodule RsolvApi.AST.AuditLogger do
  @moduledoc """
  ...
  def init(opts) do
    ...
    state = %__MODULE__{
      buffer: [],
      storage_backend: opts[:storage_backend] || :ets,  # Default to ETS for now
      retention_config: %{
        max_age_days: 30,
  ...
  def handle_cast({:log_event, event}, state) do
    ...
    state = if length(new_buffer) >= @buffer_size do
  ...
  defp sanitize_metadata(metadata) do
    Enum.reduce(metadata, %{}, fn {key, value}, acc ->
      sanitized_value = if is_sensitive_field?(key) do
        "[REDACTED]"
      else
        value
      end
  ...
  defp query_from_buffer(criteria) do
    all_events = :ets.tab2list(:audit_log_buffer)
  ...
  defp calculate_metrics do
    ...
    # Count by event type
    event_counts = Enum.reduce(all_events, %{}, fn event, acc ->
