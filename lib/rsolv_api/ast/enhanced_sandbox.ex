defmodule RsolvApi.AST.EnhancedSandbox do
  @moduledoc """
  ...
  def create_enhanced_config(parser_type, options \\ %{}) do
    ...
    enhanced_limits = Map.merge(base_config.limits, %{
      # Stricter memory limits for MVP
      max_heap_size: 32_000_000,    # 128MB (was 256MB)
      max_reductions: 1_000_000,    # Half the CPU time
      timeout_ms: 15_000,           # 15 seconds (was 30)
      max_message_queue: 100,       # Prevent message flooding
      max_processes: 10             # Limit subprocess spawning
    })
    
    enhanced_spawn_opts = base_config.spawn_opts ++ [
      priority: :low,               # Lower priority than main app
      min_heap_size: 233,          # Start small
      min_bin_vheap_size: 46422,   # Binary heap control
  ...
  defp sanitize_input(input, _language) do
    ...
    |> String.slice(0, 10 * 1024 * 1024)  # Enforce max size
  ...
  def get_security_metrics() do
    events = case :ets.whereis(:security_events) do
      :undefined -> []
