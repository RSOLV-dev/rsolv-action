defmodule RsolvWeb.Plugs.FeatureFlagPlug do
  ...
  def call(conn, %{feature: feature, fallback_url: fallback_url, message: message}) do
    if FeatureFlags.enabled?(feature) do
      ...
      Logger.info("Access attempt to disabled feature", metadata: %{
        feature: feature,
        path: conn.request_path,
        remote_ip: format_ip(conn.remote_ip)
      })
  ...
  def admin_dashboard, do: init(feature: :admin_dashboard, message: "Admin dashboard is not available")
  def metrics_dashboard, do: init(feature: :metrics_dashboard, message: "Metrics dashboard is not available")
  def feedback_dashboard, do: init(feature: :feedback_dashboard, message: "Feedback dashboard is not available")
end