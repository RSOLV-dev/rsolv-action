defmodule RsolvWeb.PageController do
  ...
  def submit_early_access(conn, params) do
    ...
    if not FeatureFlags.enabled?(:early_access_signup) do
      ...
      # Redirect to home page with appropriate message
      conn
      |> put_flash(:info, "Early access registrations are currently paused. Please check back later!")
      |> redirect(to: ~p"/")
      |> halt()
    else
      ...
      Logger.info("Form submission received", metadata: %{
        timestamp: DateTime.utc_now() |> DateTime.to_string()
      })
      ...
      case validation_result do
        {:ok, _} ->
          ...
          if FeatureFlags.enabled?(:welcome_email_sequence) do
            ...
            Logger.info("[PAGE CONTROLLER] About to call EmailSequence.start_early_access_onboarding_sequence",
              email: email,
              first_name: first_name,
              timestamp: DateTime.utc_now() |> DateTime.to_string()
            )
            
            sequence_result = RsolvWeb.Services.EmailSequence.start_early_access_onboarding_sequence(email, first_name)
            
            Logger.info("[PAGE CONTROLLER] EmailSequence.start_early_access_onboarding_sequence returned",
        ...
        {:error, error_message} ->
          ...
          error_data = Map.merge(tracking_data, %{
  ...
  defp extract_utm_params_from_conn(conn) do
    ...
    result = Enum.reduce(utm_params, initial_utm, fn {param_name, key}, acc ->
      cond do
  ...
  defp extract_tracking_data(conn) do
    ...
    user_id = get_session(conn, :tracking_id) || 
  ...
  defp deliver_and_normalize(email_struct) do
    ...
    delivered_email = case result do
      {:ok, email} -> email
