defmodule RsolvWeb.DashboardLive do
  ...
  defp fetch_dashboard_data(socket) do
    try do
      ...
      data =
        try do
          case tab do
            ...
            "conversions" ->
              fetch_conversion_data(since)

            "traffic" ->
              fetch_traffic_data(since)

            "engagement" ->
              fetch_engagement_data(since)
      ...
      average_session_duration =
        get_metric_safely("session_duration", fn -> get_average_session_duration(since) end)
    ...
    try do
      ...
      safe_period =
        if is_binary(period) do
          ...
          # Default to 7 days for invalid input
          "7d"
        end
  ...
  defp fetch_overview_data(since) do
    ...
    visitors_data =
      case Analytics.query_data(:page_views, since: since, group_by: :timestamp) do
        {:ok, data} -> data
        _ -> %{}
      end
    ...
    conversions_data =
      case Analytics.query_data(:conversions, since: since, group_by: :timestamp) do
        {:ok, data} -> data
        _ -> %{}
      end
    ...
    utm_data =
      case Analytics.query_data(:page_views, since: since, group_by: :utm_source) do
        {:ok, data} -> data
        _ -> %{}
      end
  ...
  defp fetch_traffic_data(since) do
    ...
    traffic_sources =
      Enum.group_by(page_views, fn entry ->
  ...
  defp fetch_engagement_data(since) do
    ...
    section_views =
      case Analytics.query_data(:events, since: since) do
        {:ok, data} -> data
        _ -> []
      end
    ...
    # Group by section id
    sections =
      Enum.group_by(section_view_events, fn entry ->
