defmodule RsolvWeb.Admin.LoginLive do
  ...
  def mount(params, session, socket) do
    try do
      ...
      Logger.info(
        "[Admin LoginLive] Mount complete, socket assigns: #{inspect(Map.keys(socket.assigns))}"
      )
  ...
  def handle_event("validate", params, socket) do
    try do
      ...
      Logger.debug(
        "[Admin LoginLive] Validate socket assigns: #{inspect(Map.keys(socket.assigns))}"
      )
      ...
      Logger.debug(
        "[Admin LoginLive] Validate - Email: #{email}, Password length: #{String.length(password)}"
      )
  ...
  def handle_event("submit", params, socket) do
    try do
      ...
      Logger.debug(
        "[Admin LoginLive] Submit socket assigns: #{inspect(Map.keys(socket.assigns))}"
      )
  ...
  def handle_info({:authenticate, email, password}, socket) do
    ...
    try do
      ...
      if customer_check do
        Logger.info(
          "[Admin LoginLive] Customer found - ID: #{customer_check.id}, has_password: #{customer_check.password_hash != nil}"
        )
      end
      ...
      case Customers.authenticate_customer_by_email_and_password(email, password) do
        {:ok, customer} ->
          Logger.info(
            "[Admin LoginLive] ✓ Authentication successful for #{email}, is_staff: #{customer.is_staff}"
          )
          ...
          if customer.is_staff do
            ...
            Logger.info(
              "[Admin LoginLive] Session stored in CustomerSessions for customer #{customer.id}"
            )
        ...
        {:error, :invalid_credentials} ->
          ...
          Logger.debug(
            "[Admin LoginLive] Auth failed - customer exists: #{customer_check != nil}, has_password: #{customer_check && customer_check.password_hash != nil}"
          )
        ...
        error ->
          Logger.error(
            "[Admin LoginLive] Unexpected authentication error for #{email}: #{inspect(error)}"
          )
        ...
        <div class="mx-auto w-full max-w-7xl lg:px-8">
          <div class="relative px-4 sm:px-8 lg:px-12">
            <div class="mx-auto max-w-sm">
              <h1 class="text-2xl font-bold text-center mb-8 text-gray-900 dark:text-gray-100">
                Admin Login
              </h1>
