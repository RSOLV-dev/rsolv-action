defmodule RsolvWeb.EarlyAccessLive do
          ...
          <%= if @error_message do %>
            ...
            <%= if @suggested_correction && @suggested_correction != @email do %>
              <p class="mt-1 text-yellow-400 text-sm">
                Did you mean <a href="#" phx-click="use_suggestion" class="underline hover:text-yellow-300"><%= @suggested_correction %></a>?
              </p>
            <% end %>
          <% end %>
        </div>
  ...
  def mount(_params, session, socket) do
    ...
    {:ok, assign(socket, 
      email: "",
      email_valid: nil,  # tri-state: nil (not validated), true, false
      error_message: nil,
      success_message: nil,
      suggested_correction: nil,
      current_path: "/signup",  # Initialize current_path
  ...
  def handle_event("submit", params, socket) do
    ...
    case validation_result do
      {:ok, _} ->
        ...
        conversion_data = Map.merge(socket.assigns.tracking_data, %{
      ...
      {:error, error_message} ->
        ...
        error_data = Map.merge(socket.assigns.tracking_data, %{
  ...
  def handle_event("input-change", %{"value" => email}, socket) do
    ...
    socket = if email == "" do
      ...
      |> assign(:error_message, nil) # Don't show errors while typing
  ...
  def handle_event("validate", %{"email" => email}, socket) do
    ...
    if email != "" do
      validation_data = Map.merge(socket.assigns.tracking_data, %{
  ...
  defp parse_user_agent(user_agent) do
    ...
    browser = cond do
      String.contains?(user_agent, "Chrome") && !String.contains?(user_agent, "Chromium") -> "chrome"
      String.contains?(user_agent, "Firefox") -> "firefox"
      String.contains?(user_agent, "Safari") && !String.contains?(user_agent, "Chrome") -> "safari"
      String.contains?(user_agent, "Edge") -> "edge"
      String.contains?(user_agent, "Opera") || String.contains?(user_agent, "OPR") -> "opera"
      true -> "other"
    end
    ...
    os = cond do
      ...
      String.contains?(user_agent, "Linux") && !String.contains?(user_agent, "Android") -> "linux"
      String.contains?(user_agent, "Android") -> "android"
      String.contains?(user_agent, "iOS") || String.contains?(user_agent, "iPhone") || String.contains?(user_agent, "iPad") -> "ios"
      true -> "other"
    end
  ...
  defp save_email_to_file(email) do
    ...
    signup_attrs = %{
      email: email,
      name: nil,  # We don't collect name in this form
      company: nil,  # We don't collect company in this form
      referral_source: "early_access_live",
      utm_source: nil,  # Could extract from tracking_data if needed
      utm_medium: nil,
      utm_campaign: nil
    }
    ...
    case EarlyAccess.create_signup(signup_attrs) do
      ...
      {:error, changeset} ->
        Logger.error("Failed to save email to database: #{email}, errors: #{inspect(changeset.errors)}")
    end
    
    # Send to Kit.com API
    case send_to_kit(email) do
      {:ok, response} ->
        Logger.info("Successfully sent email to Kit.com: #{email}, response: #{inspect(response)}")
    ...
    if is_nil(kit_api_key) do
      ...
      case HTTPoison.post(url, body, headers, [recv_timeout: 10000]) do
        ...
        {:ok, %HTTPoison.Response{status_code: status_code} = response} ->
          ...
          Logger.error("Kit.com API error: Status #{status_code}, Body: #{inspect(response.body)}")
