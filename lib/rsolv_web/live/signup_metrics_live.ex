defmodule RsolvWeb.SignupMetricsLive do
  ...
  defp fetch_signup_metrics(socket) do
    try do
      ...
      average_daily =
        get_metric_safely("average_daily", fn -> get_average_daily_signups(since, until) end)

      source_breakdown =
        get_metric_safely("source_breakdown", fn -> get_source_breakdown(since) end)
  ...
  defp fetch_signup_overview(since, until) do
    ...
    conversions =
      case Analytics.query_data(:conversions, since: since, until: until) do
        {:ok, data} -> data
        _ -> []
      end
    ...
    form_events =
      case Analytics.query_data(:form_events, since: since, until: until) do
        {:ok, data} -> data
        _ -> []
      end

    form_submissions =
      Enum.filter(form_events, fn event ->
  ...
  defp fetch_signup_funnel(since, until) do
    ...
    page_views =
      case Analytics.query_data(:page_views, since: since, until: until) do
        {:ok, data} -> data
        _ -> []
      end
    ...
    funnel_stages = [
      ...
      %{
        name: "Form Views",
        count: form_views,
        rate: calculate_percentage(form_views, unique_visitors)
      },
      %{
        name: "Form Submissions",
        count: form_submissions,
        rate: calculate_percentage(form_submissions, form_views)
      },
      %{
        name: "Form Success",
        count: form_success,
        rate: calculate_percentage(form_success, form_submissions)
      },
      %{
        name: "Completed Signups",
        count: signups,
        rate: calculate_percentage(signups, form_success)
      }
    ]

    # Calculate drop-off points
    drop_offs =
      Enum.map(0..(length(funnel_stages) - 2), fn i ->
  ...
  defp check_convertkit_integration do
    ...
    %{
      ...
      # Assuming it's active if configured
      status: "active"
    }
  end
  ...
  defp check_plausible_integration do
    ...
    %{
      # We'd need to check the actual layout for this
      configured: true,
      status: "active"
    }
  end
  ...
  defp create_weekly_counts(since, until, grouped_data) do
    ...
    days_to_monday =
      case Date.day_of_week(since) do
        # Monday
        1 -> 0
  ...
  defp count_by_day_of_week(signups) do
    ...
    day_counts =
      Enum.reduce(1..7, %{}, fn day, acc ->
        Map.put(acc, day, 0)
      end)

    # Count by day
    counts =
      Enum.reduce(signups, day_counts, fn signup, acc ->
  ...
  defp count_by_hour_of_day(signups) do
    ...
    # Count by hour
    counts =
      Enum.reduce(signups, hour_counts, fn signup, acc ->
  ...
  defp format_time_series_data(grouped_data, label) do
    try do
      ...
      if is_map(grouped_data) do
        ...
        # Remove error items
        |> Enum.filter(fn item -> item.date != "error" end)
        |> Enum.sort_by(& &1.date)
      else
  ...
  defp format_utm_sources(utm_data) do
    try do
      ...
      if is_map(utm_data) do
        ...
        # Remove error items
        |> Enum.filter(fn item -> item.source != "error" end)
