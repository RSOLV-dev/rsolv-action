      ...
      <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class={"md:hidden #{if @mobile_menu_open, do: "", else: "hidden"}"} id="mobile-menu">
          <div class="px-2 pt-2 pb-3 space-y-1">
            <%= if @is_homepage do %>
              <a href="#features" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Features</a>
              <a href="#pricing" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Pricing</a>
              <a href="#faq" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>FAQ</a>
            <% else %>
              <a href="/" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Home</a>
              <a href="/#features" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Features</a>
              <a href="/#pricing" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Pricing</a>
            <% end %>
            <%= if @show_blog_link do %>
              <a href="/blog" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Blog</a>
            <% end %>
            <a href="/feedback" phx-click={@socket && "close_mobile_menu"} class={"block px-3 py-2 rounded-md text-base font-medium #{@link_color}"}>Feedback</a>
          </div>
        </div>
      </div>
    </header>
