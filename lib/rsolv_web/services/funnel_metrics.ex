defmodule RsolvWeb.Services.FunnelMetrics do
  ...
  @funnel_stages [
    # Initial landing page visit
    "page_visit",
    # User viewed the signup form
    "signup_view",
    # User interacted with the form (typing)
    "form_interact",
    ...
    "confirmation_sent",
    # User opened confirmation email
    "confirmation_open",
  ...
  defp process_session_funnel(session_id, events) do
    ...
    {reached_stages, _} =
      Enum.reduce(@funnel_stages, {[], nil}, fn stage, {stages, last_time} ->
    ...
    case stage do
      ...
      "signup_view" ->
        Enum.find(events, &(&1["type"] == "page_view" && &1["path"] == "/signup")) ||
          Enum.find(
            events,
            &(&1["type"] == "form_event" && &1["event"] == "form_view" && &1["form"] == "signup")
          )

      "form_interact" ->
        Enum.find(
          events,
          &(&1["type"] == "form_event" && &1["event"] == "form_change" && &1["form"] == "signup")
        )

      "form_submit" ->
        Enum.find(
          events,
          &(&1["type"] == "form_event" && &1["event"] == "form_submit" && &1["form"] == "signup")
        )

      "validation_pass" ->
        Enum.find(
          events,
          &(&1["type"] == "form_event" && &1["event"] == "form_valid" && &1["form"] == "signup")
        )
      ...
      "confirmation_open" ->
        Enum.find(
          events,
          &(&1["type"] == "email_event" && &1["event"] == "open" && &1["type"] == "confirmation")
        )
