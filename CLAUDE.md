...
For new developers, use the interactive environment setup wizard:

```bash
# Run the wizard to configure .env file
mix dev.env.setup

# Then run full project setup
mix setup
```

The wizard will:
- ✅ Copy `.env.example` to `.env`
- 🔐 Generate secure `SECRET_KEY_BASE` automatically
- 🤖 Optionally prompt for AI provider keys (Anthropic, OpenAI, OpenRouter)
- 🗄️ Test database connection and suggest fixes
- 📧 Configure optional services (Postmark, ConvertKit)

### Environment Configuration

All environment variables are documented in `.env.example`. Key requirements:

**Required Variables:**
- `DATABASE_URL` - PostgreSQL connection (e.g., `postgresql://postgres:postgres@localhost/rsolv_dev`)
- `SECRET_KEY_BASE` - 64+ character secret (auto-generated by wizard or via `mix phx.gen.secret`)
- `DATABASE_SSL` - Set to `false` for local dev, `true` for production

**Optional but Recommended:**
- `ANTHROPIC_API_KEY` - For Claude AI features ([get key](https://console.anthropic.com/))
- `OPENAI_API_KEY` - For OpenAI features ([get key](https://platform.openai.com/api-keys))
- `POSTMARK_API_KEY` - For transactional emails ([get key](https://postmarkapp.com/))

**When making changes to runtime configuration:**
- Always update `.env.example` to keep it in sync with `config/runtime.exs`
- Document new variables with comments explaining their purpose and where to get values
